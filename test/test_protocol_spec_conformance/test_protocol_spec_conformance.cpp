// Protocol-vector regression — the firmware's decode behavior tested against
// vectors in test/fixtures/protocol_spec_tables.h, held separately so the decode
// cannot author its own expected values. Provenance of those vectors is mixed and
// is recorded per-table in the fixture; a table without a citation there is
// unsourced and pins current behavior only.
//
...
// this display. Keeping independent expectations makes a belief error visible
// instead of leaving a self-consistent green suite.
...
#include "../../include/display_visual_contract.h"
...
// Bytes outside the valid contiguous-from-LSB domain are not strengths the V1
// could have sent, so the passthrough rule does not reach them: they take the
// overflow sentinel. A literal bit count here would understate a live threat on
// exactly the corrupt input that most needs to fail loud.
void test_led_bitmap_outside_the_valid_domain_takes_the_sentinel() {
    // Every byte here is absent from Table 9.1. 0x7F and 0xFF are NOT in this
    // list: they are defined rows of the table (7 and 8 LEDs) and are covered
    // by test_led_bitmap_decode_matches_spec_table above.
    const uint8_t samples[] = {0x80, 0xAA, 0x55, 0x02, 0xF0, 0x81, 0x3E, 0x7E, 0xFE};
...
void assertStrengthTable(uint8_t bandByte,
                         ...
                         size_t tableLen) {
    // The parser preserves VR's diagnostic 0..8 strength; visual projection is
    // tested separately at the renderer contract boundary.
    for (size_t i = 0; i < tableLen; ++i) {
        const uint8_t raw = table[i].minRaw;
        const uint8_t vrBars = table[i].vrBars;
        const uint8_t expectAt = vrBars;
        char msg[64];
        ...
        const uint8_t expectBelow = (below == 0x00) ? 0 : vrBelow;
...
void test_alert_row_layout_matches_spec_table() {
    // Pin the offsets to the fixture layout (compile-time facts from the
    ...
    // Frequency must be read as MSB<<8 | LSB from the fixture offsets.
    PacketParser parser;
    ...
    TEST_ASSERT_EQUAL_UINT8(8, alert.frontStrength);
    TEST_ASSERT_EQUAL_UINT8(0, alert.rearStrength);
}
...
//
// Valentine's Law surface (docs/VALENTINE_PHILOSOPHY.md). These bits are the
// only path by which this firmware can change what the Valentine One itself
// detects, so a wrong row here is the most consequential failure available to
// the project: it would silently disable a band on the detector, and the
// display would faithfully render the resulting silence.
//
// v1_profile_push_policy flips LASER (byte0/0x08) and auto-push rewrites
// MUTE_TO_MUTE_VOLUME (byte0/0x10). A wrong row here means
// a profile push could silently disable a detection band. These tests drive
// the real V1UserSettings accessors against the fixture map.
// ============================================================================
...
void test_user_bytes_bool_rows_match_spec_table() {
    ...
    // Every bool accessor is pinned by a fixture row and vice versa.
...
void test_laser_strength_is_full_scale_regardless_of_raw() {
    ...
    for (uint8_t raw : raws) {
        ...
        TEST_ASSERT_EQUAL_UINT8_MESSAGE(
            protocol_spec::kLaserVrBars,
            parseAlertFrontStrength(0x01 | 0x20, raw, nullptr, nullptr), msg);
    }
}

void test_card_projection_matches_six_segment_contract() {
    for (uint8_t vrBars = 0; vrBars <= 8; ++vrBars) {
        TEST_ASSERT_EQUAL_UINT8(protocol_spec::kVrToCardBars[vrBars],
                                DisplayVisualContract::projectVrBarsToSix(vrBars));
    }
    TEST_ASSERT_EQUAL_UINT8(6, DisplayVisualContract::projectVrBarsToSix(0xFF));
}
...
int main() {
    ...
    RUN_TEST(test_card_projection_matches_six_segment_contract);
