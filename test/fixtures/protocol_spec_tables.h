...
// Protocol conformance vectors, held apart from the parser so that a wrong belief
// in the decode cannot also author its own expected values.
//
// Provenance is mixed, and each table states its own. A table that cites a document
// and commit was transcribed from that source and is independent evidence that the
// decode matches the wire. A table with no citation is UNSOURCED: it came from the
// `docs/` tree retired in B14/B15 and has no surviving external reference.
//
// UNSOURCED bounds what a green run proves; it does not mean the table is idle.
// These tables still catch accidental change to the decode, still prove one field's
// accessor cannot disturb another's bits, and still hold the accessor set and the
// fixture rows in one-to-one correspondence — they earn their place under §5(b)
// on that alone. What they cannot do is convict a belief the decode and the fixture
// share. Sourcing one promotes it from regression pin to evidence; deleting one
// gives up the pin too.
...
// ESP Specification 3.015, Table 9.1 "Alert Table to Bar Graph Map" (p.34),
// ValentineResearch/AndroidESPLibrary2 @ a7cd958. All nine rows, verbatim.
// The table is generation-neutral: the specification splits Tables 9.3/9.4 by
// generation and does not split this one.
...
// Bytes absent from Table 9.1 take VR's getNumberOfLEDS() default-branch value
// (InfDisplayData.java:648 @ a7cd958).
...
// vrBars (index 0..8) -> six-cell secondary-card projection.
