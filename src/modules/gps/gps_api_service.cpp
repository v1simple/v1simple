...
void handleApiConfigSave(WebServer& server,
    ...
    if (body["gpsEnablePinActiveHigh"].is<bool>()) {
        update.hasGpsEnablePinActiveHigh = true;
        update.gpsEnablePinActiveHigh = body["gpsEnablePinActiveHigh"].as<bool>();
    }
    ...
    if (update.hasGpsEnabled) {
        ...
        // Baud or polarity changed: restart UART with new parameters.
