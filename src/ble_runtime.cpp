...
void V1BLEClient::process() {
    ...
    if (pendingConnectStateUpdate_) {
        ...
        if (lock.locked()) {
            const uint32_t edgeGeneration = pendingConnectStateGeneration_.load(std::memory_order_relaxed);
    ...
    const bool proxyConnected =
        proxyClientConnected_.load(std::memory_order_relaxed) || (pServer_ && pServer_->getConnectedCount() > 0);
