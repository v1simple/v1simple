...
struct PerfExtendedMetrics {
    ...
    // Count of InfDisplayData LED bitmaps outside the valid contiguous-from-LSB
    // domain. Those cannot be something the V1 said, so the meter fails loud at
    // max bars rather than rendering them literally; this counts how often that
    ...
    // update(priority,...)'s flush decision.
...
// Count one LED bitmap outside the valid domain (non-contiguous from LSB).
