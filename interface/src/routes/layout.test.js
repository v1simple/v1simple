...
function childSnippet(text = 'Route content') {
	return createRawSnippet(() => ({ render: () => `<section aria-label="test child">${text}</section>` }));
}
...
describe('root layout', () => {
	...
	it('renders route content and marks the active nav path', () => {
		installFetchMock([{ method: 'GET', match: '/api/device/settings', respond: jsonResponse({}) }], jsonResponse({}));
		pageStore.setPath('/colors');

		const { unmount } = renderLayout();

		expect(screen.getByRole('region', { name: /test child/i })).toHaveTextContent('Route content');
	...
	it('shows and dismisses the default-password warning', async () => {
		vi.useFakeTimers();
		installFetchMock(
			[{ method: 'GET', match: '/api/device/settings', respond: jsonResponse({ isDefaultPassword: true }) }],
			jsonResponse({})
		);
		const { unmount } = renderLayout();
