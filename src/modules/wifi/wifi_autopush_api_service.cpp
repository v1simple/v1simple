...
void handleApiPushNow(WebServer& server, const Runtime& runtime, bool (*checkRateLimit)(void* ctx),
    ...
    case PushNowQueueResult::QUEUED:
        server.send(202, "application/json",
                    "{\"success\":true,\"queued\":true,\"result\":\"queued\","
                    "\"statusUrl\":\"/api/autopush/status\"}");
        return;
    ...
    case PushNowQueueResult::PROFILE_LOAD_FAILED:
        ...
        return;
    case PushNowQueueResult::INVALID_VOLUME_PAIR:
        server.send(400, "application/json", "{\"error\":\"Main and mute volume must be configured together\"}");
        return;
    default:
        server.send(500, "application/json", "{\"error\":\"Failed to queue push\"}");
        return;
    }
}
