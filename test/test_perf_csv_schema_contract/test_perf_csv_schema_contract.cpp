...
void test_perf_csv_schema_version_matches_current_header() {
    ...
    TEST_ASSERT_NOT_EQUAL(
        std::string::npos,
        source.find("static constexpr uint32_t PERF_CSV_SCHEMA_VERSION = 45;"));
}
...
void test_perf_csv_header_includes_car_mode_alp_silence_metric() {
    ...
    TEST_ASSERT_NOT_EQUAL(
        std::string::npos,
        source.find(
            "powerAutoPowerTimerStart,powerAutoPowerTimerCancel,powerAutoPowerTimerExpire,powerCarModeAlpSilenceExpire,powerCriticalWarn"));
}
...
void test_perf_csv_header_includes_connect_burst_attribution_columns() {
    ...
    TEST_ASSERT_NOT_EQUAL(
        std::string::npos,
        source.find(
            "bleFirstRxMs,bleFollowupRequestAlertMax_us,bleFollowupRequestVersionMax_us,bleConnectStableCallbackMax_us,bleProxyStartMax_us,displayGapRecoverMax_us,displayFullRenderCount"));
}
...
void test_perf_csv_header_includes_partial_flush_region_union_columns() {
    ...
    TEST_ASSERT_NOT_EQUAL(
        std::string::npos,
        source.find(
            "displayRedrawReasonFlashTickCount,displayRedrawReasonFullFlushForRedrawCount,displayRedrawReasonCacheHitSkipFlushCount,displayRedrawReasonUnionExceedsCapCount,displayRedrawReasonPartialRegionFlushCount"));
}
...
void test_perf_metrics_exports_render_and_connect_burst_sources() {
    ...
    TEST_ASSERT_NOT_EQUAL(
        std::string::npos,
        source.find("flat.bleProxyStartMaxUs = metrics.bleProxyStartMaxUs;"));
    TEST_ASSERT_NOT_EQUAL(
        std::string::npos,
    ...
    TEST_ASSERT_NOT_EQUAL(
        std::string::npos,
        source.find("flat.displayPartialFlushWouldFullRows128Count = metrics.displayPartialFlushWouldFullRows128Count;"));
    TEST_ASSERT_NOT_EQUAL(
        std::string::npos,
...
void test_perf_csv_header_includes_gps_utc_and_observability_columns() {
    ...
    TEST_ASSERT_NOT_EQUAL(std::string::npos,
        sdLogger.find("gpsHasFix,gpsStableHasFix,gpsEnableTransitions"));
    ...
    TEST_ASSERT_NOT_EQUAL(std::string::npos,
        metrics.find("flat.gpsEnableTransitions     = gpsStatus.enableTransitions;"));
}
