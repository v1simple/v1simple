...
const DisplayPreviewModule::PreviewStep DisplayPreviewModule::STEPS[] = {
    ...
    // Ka 34.700 front (priority) + K 24.150 side (full six-cell card from VR 8)
    {BAND_KA,  DIR_FRONT, 34700,  5,      0,      FLAG_NONE, BAND_K, DIR_SIDE, 24150,  8,      0,
    ...
    // ── Meter segment-state coverage ────────────────────────────────────
    // The visual re-acceptance exercises the weakest and strongest states of
    // the restored six-segment meter.
    //
    // 1 is the weakest lit state. 0 segments is the resting screen rather than
    // an alert, so it is exercised by the no-alert frames, not authored here.
    {BAND_KA, DIR_FRONT, 34700, 1, 0, FLAG_NONE, NO_SEC, NO_THIRD, '1', NO_CHG, NO_CHG, NO_CHG, NO_CHG, NO_CHG, NO_CHG,
     NO_CHG, NO_CHG, nullptr},

    // 6 fills all six segments in V1Simple's preview meter.
    {BAND_KA, DIR_FRONT, 34700, 6, 0, FLAG_NONE, NO_SEC, NO_THIRD, '1', NO_CHG, NO_CHG, NO_CHG, NO_CHG, NO_CHG, NO_CHG,
     NO_CHG, NO_CHG, nullptr},
...
uint8_t maxPreviewBars(uint8_t frontBars, uint8_t rearBars) {
    return std::min<uint8_t>(DisplayLayout::MAIN_SIGNAL_BAR_COUNT, std::max(frontBars, rearBars));
}
...
void fillResolvedAlert(DisplayPreviewModule::ResolvedAlert& out, Band band, Direction dir, uint32_t freqMHz,
    ...
    // Derived through the renderer's own selection so the reported count is by
    // construction the count the card will paint. No parallel math here.
    AlertData asRendered{};
    asRendered.direction = dir;
    asRendered.frontStrength = frontBars;
    asRendered.rearStrength = rearBars;
    out.cardBarCount = DisplayVisualContract::alertMeterBars(asRendered);
...
void buildResolvedStepFromCarry(int index, const DisplayPreviewModule::PreviewStep& step,
    ...
    // The main meter mirrors V1's six-bit display bitmap domain.
    out.mainMeterCount = maxPreviewBars(step.frontBars, step.rearBars);
