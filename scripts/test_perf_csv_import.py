...
HEADER_COLUMNS = list(
    dict.fromkeys(
        [
            ...
            *import_perf_csv.CSV_DELTA_COLUMNS.values(),
            *import_perf_csv.CSV_PEAK_DIAGNOSTIC_COLUMNS.values(),
            *import_perf_csv.CSV_PEAK_ONLY_COLUMNS.values(),
            *(column for column, _metric in import_perf_csv.DISPLAY_COUNTER_DELTA_MAPPINGS),
            *(column for column, _label in import_perf_csv.OBD_SYNC_CALL_COLUMNS),
            *(column for column, _label in import_perf_csv.WIFI_SUBPHASE_COLUMNS),
            *(column for column, _label in import_perf_csv.CONNECT_BURST_BLE_COLUMNS),
            *(column for column, _label in import_perf_csv.CONNECT_BURST_DISPLAY_COLUMNS),
        ]
    )
)
...
def test_truncated_row_is_rejected(tmpdir: Path) -> None:
    csv_path = tmpdir / "truncated.csv"
    out_dir = tmpdir / "truncated_out"
    session = make_session(
        seq=1,
        token="TRUNC001",
        schema=CURRENT_HEADER_SCHEMA,
    ...
    result = run_import(csv_path, out_dir)
    assert_true(result.returncode == 3, f"truncated row should fail import: rc={result.returncode}")
    assert_true("Truncated CSV row" in result.stderr, f"missing truncated-row error: {result.stderr}")
    assert_true(
        f"expected {len(HEADER_COLUMNS)} columns, got {len(HEADER_COLUMNS) - 1}" in result.stderr,
        f"truncated-row error should report widths: {result.stderr}",
    )


def test_gps_utc_timestamp_is_accepted(tmpdir: Path) -> None:
    csv_path = tmpdir / "gps_utc.csv"
    out_dir = tmpdir / "gps_utc_out"
    header_columns = ["millis", "utc", *HEADER_COLUMNS[1:]]
    session = make_session(
        seq=1,
        token="GPSUTC01",
        schema=37,
    ...
    for index, row in enumerate(rows):
        assert isinstance(row, dict)
        row["utc"] = f"2026-08-07T12:00:{index:02d}.123Z"
    write_capture(csv_path, header_columns=header_columns, sessions=[session])

    result = run_import(csv_path, out_dir)
    assert_true(result.returncode != 3, f"GPS UTC import failed: {result.stderr}")
    parsed_sessions = import_perf_csv.load_sessions(csv_path)
    assert_true("utc" not in parsed_sessions[0][1][0], "UTC metadata should not enter the numeric metric row")
