...
def select_all_frames(stimulus: list[dict], rows: list[dict],
    ...
    for index, row in enumerate(rows):
        ...
        if not any(start <= capture < end for start, end in bounds):
            continue
        require(len(samples) < MAX_SAMPLES, "selection exceeds 5000 frames; narrow the explicit range")
...
def analyze(run: Path, out: Path, ranges: list[tuple[float, float]] | None, cadence: float,
            configuration: Path | None = None, transition_only: bool = False, all_frames: bool = False) -> dict:
    samples, errors, evidence, data = [], [], {}, None
    ...
    try:
        ...
        save_json(out / "selection.json", dict(schema_version=1, identity=data["identity"], ranges=ranges,
                                               ...
                                               transition_offsets_seconds=[] if all_frames else PROBES, samples=samples))
        by_index = {}
        for sample in samples:
...
def write_report(out: Path, result: dict) -> None:
    ...
    title = "Consecutive-frame encounter" if result["selection_mode"] == "all_recorded_frames" else "Sampled encounter"
