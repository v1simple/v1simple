...
bool TouchUiModule::process(unsigned long nowMs, bool bootPressed) {
    ...
    if (!bootPressed && bootWasPressed_) {
        ...
        if (triggerObdPair) {
            ...
            const bool isDoublePress = (lastShortPressReleaseMs_ != 0) &&
                                       (nowMs - lastShortPressReleaseMs_) < DOUBLE_PRESS_WINDOW_MS;
            lastShortPressReleaseMs_ = nowMs;
            if (isDoublePress) {
                lastShortPressReleaseMs_ = 0;  // Reset: next press can't triple-trigger
                if (brightnessAdjustMode_) exitAdjustModeAndSave();
                if (settings_) settings_->setStealthEnabled(!settings_->get().stealthEnabled);
