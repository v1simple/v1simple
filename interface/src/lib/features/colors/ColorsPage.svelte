<script>
	...
	const STOP_FIELDS = [
		{ prop: 'bottom', key: 'stopBottom', label: 'Bottom (weakest)', pickerLabel: 'Bottom Stop' },
			...
			<div class="card-body">
				...
				<div class="field-control">
					<div class="flex items-center gap-4">
						...
						<span class="text-sm font-mono w-12 text-right">{Math.max(1, Math.round((colors.brightness / 255) * 100))}%</span>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="surface-card">
			<div class="card-body">
				...
				{#if barEditor.mode === 'simple'}
					<div class="grid grid-cols-3 gap-2">
						{#each STOP_FIELDS as field}
							...
							>
								<div class="w-8 h-3 rounded" style={`background-color: ${rgb565ToHex(barEditor[field.prop])}`}></div>
							</ColorControl>
						{/each}
					</div>
					<button type="button" class="btn btn-ghost btn-xs self-start" onclick={() => switchBarEditorMode('advanced')}>
				...
				{:else}
					...
					<button type="button" class="btn btn-ghost btn-xs self-start" onclick={() => switchBarEditorMode('simple')}>
						Use simple 3-stop gradient (editing a stop blends these colors into a smooth ramp)
					</button>
				{/if}
				<div class="flex justify-center mt-3">
					...
					<div class="flex flex-col-reverse gap-1">
						{#each sampleBarRampRgb565(colors) as barColor}
							<div class="w-12 h-2 rounded" style={`background-color: ${rgb565ToHex(barColor)}`}></div>
						{/each}
					</div>
				</div>
			</div>
