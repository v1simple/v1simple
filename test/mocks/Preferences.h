...
// suites exercise the firmware's partial-fill guards (e.g. the nvsValid
// written-last sentinel) instead of only the blanket failWrites() switch.
inline size_t& entryLimit() {
    static size_t g_entryLimit = 0;
    return g_entryLimit;
}
...
inline size_t totalEntryCount() {
    size_t count = 0;
    for (const auto& ns : store()) {
        count += ns.second.size();
    }
    return count;
}

inline bool wouldExceedEntryLimit(const std::string& ns, const char* key) {
    const size_t limit = entryLimit();
    if (limit == 0 || !key) {
        return false;
    }
    if (find(ns, key) != nullptr) {
        return false;  // in-place update of an existing key
    }
    return totalEntryCount() >= limit;
}
