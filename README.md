...
The maintenance UI handles WiFi network selection, offline V1 profile authoring, display/quiet/audio settings, ALP/OBD/GPS settings, backup/restore, logging configuration, and diagnostics. Logs themselves are written to SD (perf CSV) and the serial console — the web UI does not display them. Live V1 push/pull and runtime OBD/GPS/ALP status are unavailable there because those runtimes are intentionally not started.
