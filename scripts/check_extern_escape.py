...
def mask_comments_and_strings(source: str) -> str:
    ...
    while i < len(source):
        ...
        if i < len(source) - 1 and source[i : i + 2] == "/*":
            ...
            if end != -1:
                # Preserve newlines so masked/unmasked line pairing stays in
                # sync — blanking them shifted every subsequent line and made
                # the guard blind to files opening with a block comment.
