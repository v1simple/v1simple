...
class TouchUiModule {
  public:
    ...
    // A higher-priority presentation owner pauses input without discarding an
    // active slider session. Gesture edges are reset so a press/release that
    // spans the warning cannot fire after ownership returns.
    void suspendForPresentationOwner();

    // Redraw the slider owner after a higher-priority presentation ends.
