...
void test_audio_stack_high_water_keeps_zero_as_a_valid_minimum() {
    std::atomic<uint32_t> minimumBytes{UINT32_MAX};

    audioRecordStackHighWater(minimumBytes, 800);
    audioRecordStackHighWater(minimumBytes, 1200);
    audioRecordStackHighWater(minimumBytes, 640);
    audioRecordStackHighWater(minimumBytes, 0);

    TEST_ASSERT_EQUAL_UINT32(0, minimumBytes.load());
}
...
int main() {
    ...
    RUN_TEST(test_audio_stack_high_water_keeps_zero_as_a_valid_minimum);
    return UNITY_END();
}
