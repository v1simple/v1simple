...
# Match strings as well as comments so comment markers inside quoted text do not
# hide later code on the same line. A line comment ends at the newline;
# `//.*$` under DOTALL would instead consume the rest of the file.
MASK_RE = re.compile(
    r'//[^\n]*|/\*.*?\*/|"(?:\\.|[^"\\])*"|\'(?:\\.|[^\'\\])*\'',
    re.DOTALL,
)
...
def mask_comments_and_strings(source: str) -> str:
    ...
    return MASK_RE.sub(_blank, source)
...
def main() -> int:
    ...
    for path in iter_source_files():
        ...
        # Blank comments and strings to avoid documentation false positives.
        # Offsets are preserved, so match indices remain valid in `text`.
        stripped = mask_comments_and_strings(text)
