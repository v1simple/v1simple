...
// rewrite. A large backup on a slow SD card can exceed the watchdog window.
// esp_task_wdt_reset() is ESP-IDF only, so applyBackupDocument() takes an
// injectable SettingsRestoreWatchdog; these
// tests count the feeds and pin their placement relative to the profile loop.
...
// A backup body with many scalar fields but no profiles verifies that feeds
// occur per phase rather than per field.
...
// A profile-free restore still feeds once per phase boundary, and the feed
...
// Feed cadence tracks the batched profile count, so a bigger backup gets more
