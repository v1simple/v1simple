...
function identity(){const evidence=report.evidence||{},runtime=evidence.runtime_identity||{},source=evidence.tooling_source||{};
 ...
 $('stats').innerHTML=[[events.length,'authored input events'],[target+' / '+events.length,'complete target observed'],[persistent,'events with contrary content'],[unknown.toLocaleString(),'unresolved field readings retained']].map(([n,label])=>'<div class="stat"><b>'+esc(n)+'</b><span>'+esc(label)+'</span></div>').join('');
...
identity();renderComparison();renderIndex();if(events.length){const requested=indexFromHash();selectEvent(requested>=0?requested:0,false);}else{$('event-content').innerHTML='<h2>No event observations</h2><p>The result contains no evaluated input events. See analysis limitations above.</p>';$('viewer').hidden=true;$('field-content').hidden=true;$('source-content').hidden=true;}
</script></body></html>'''
