...
LoopConnectionEarlyResult LoopConnectionEarlyModule::process(const LoopConnectionEarlyContext& ctx) {
    ...
    if (snapshot.requestShowInitialScanning) {
        if (!ctx.presentationSuppressed && providers.showInitialScanning) {
            providers.showInitialScanning(providers.scanningContext);
        }
        // Preserve single-shot behavior when the request is emitted. A
        // higher-priority owner will restore an authoritative screen when it
        // releases, so this boot-only request must not replay afterward.
    ...
    if (providers.runDisplayEarly && !ctx.presentationSuppressed) {
