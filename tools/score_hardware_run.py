...
def _regression_state(
    ...
    """Classify a metric against its baseline.

    ``spread`` is the observed run-to-run range for this metric across the
    baseline samples, measured on an unchanged binary. Peak metrics move on
    their own: a single sample is not a reference point, and comparing against
    one turns ordinary variance into a reported regression. Folding the observed
    spread into the threshold means a delta is only called a regression once it
    exceeds what the same firmware already does by itself.
    """
    if baseline is None:
    ...
    if spread is not None and spread > 0:
        threshold = max(threshold, spread)
...
def score_run(
    ...
    # Observed run-to-run range per metric across the baseline samples.
    baseline_spreads: dict[tuple[str, str], float] = {}
    ...
    if baseline_candidates:
        ...
        # regressions in soak runs that started over SLA. An inline filter at
        # the upstream candidate-collection site would reintroduce the silent
        # degradation this fallback prevents.
        passing_candidates = [
            candidate
        ...
        for key, values in grouped_baselines.items():
            ...
            # With several samples of the same binary, the range between them is
            # the metric's own noise. One sample yields no spread, which leaves
            # the threshold exactly as it was before.
            if len(values) > 1:
                baseline_spreads[key] = max(values) - min(values)
