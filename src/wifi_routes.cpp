...
bool WiFiManager::setupWebServer() {
    ...
    server_.on("/api/display/visual/steps", HTTP_GET,
               [this]() { WifiDisplayVisualApiService::handleSteps(server_, makeDisplayVisualRuntime()); });
    server_.on("/api/display/visual/layout", HTTP_GET,
               [this]() { WifiDisplayVisualApiService::handleLayout(server_, makeDisplayVisualRuntime()); });
    server_.on("/api/display/visual/pin", HTTP_POST, [this]() {
        if (!requireMaintenanceApiWriteHeader())
            return;
        WifiDisplayVisualApiService::handlePin(
            server_, makeDisplayVisualRuntime(),
            [](void* ctx) { return static_cast<WiFiManager*>(ctx)->checkRateLimit(); }, this);
    });
    server_.on("/api/display/visual/framebuffer", HTTP_GET,
               [this]() { WifiDisplayVisualApiService::handleFramebuffer(server_, makeDisplayVisualRuntime()); });
    server_.on("/api/display/visual/flushshadow", HTTP_GET,
               [this]() { WifiDisplayVisualApiService::handleFlushShadow(server_, makeDisplayVisualRuntime()); });
