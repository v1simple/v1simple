...
routes do not add sessions, per-request authentication, or CSRF tokens. Change
the default AP password before using maintenance mode in any shared RF
environment. Treat
backup exports as sensitive when credential export is explicitly requested,
because they can include saved WiFi passwords. While connected to the
maintenance AP, avoid browsing untrusted pages in the same browser session:
without CSRF tokens, another page could attempt local requests to the
maintenance IP.
