...
| `wifi_boot_policy.h` (header-only) | Boot-time policy — when may WiFi auto-start? |
...
| `wifi_sta_slot_policy.{h,cpp}` | Saved STA slot selection/priority policy. |
...
| `wifi_json_document.h` (header-only) | PSRAM-aware ArduinoJson allocator. |
| `wifi_api_response.h` (header-only) | JSON-response helpers shared across handlers. |
...
| `wifi_audio_api_service.{h,cpp}` | `/api/audio/settings`. |
...
The public surface is a single method: `ensureCallbacksConfigured()` — the loop-side pump lives in `WifiRuntimeModule`, not here.
