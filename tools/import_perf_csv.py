...
def peak_limit_map(profile: str) -> dict[str, float]:
    """Return applicable peak limits from the kept hardware metric catalog."""

    payload = json.loads(CATALOG_PATH.read_text(encoding="utf-8"))
    selected: dict[str, tuple[int, float]] = {}
    for entry in payload.get("metrics") or []:
        if not isinstance(entry, dict) or entry.get("run_kind") != "real_fw_soak":
            continue
        metric = str(entry.get("metric") or "")
        column = CSV_PEAK_DIAGNOSTIC_COLUMNS.get(metric)
        limit = entry.get("absolute_max")
        selector = entry.get("selector") or {}
        if column is None or not isinstance(limit, (int, float)) or not isinstance(selector, dict):
            continue
        if any(key != "suite_or_profile" for key in selector):
            continue
        if selector.get("suite_or_profile", profile) != profile:
            continue
        specificity = len(selector)
        prior = selected.get(column)
        if prior is None or specificity > prior[0]:
            selected[column] = (specificity, float(limit))
    return {column: limit for column, (_specificity, limit) in selected.items()}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=("drive_wifi_off", "drive_wifi_ap"),
        help="Hardware catalog profile: drive_wifi_off (default) or drive_wifi_ap",
    )
    parser.add_argument(
        "--segment",
        default="",
...
def parse_session_meta(line: str) -> SessionMeta:
    ...
    return SessionMeta(
        ...
        bootId=int(values.get("bootId", "0")),
        uptime_ms=int(values.get("uptime_ms", "0")),
        token=values.get("token", ""),
        schema=int(values.get("schema", "0")),
    )
...
def duration_s(rows: list[dict[str, int]]) -> float:
    if not rows:
        return 0.0
    return max(0.001, _duration_ms(rows) / 1000.0)
...
def choose_session(
    ...
    if index:
        if index < 1 or index > len(sessions):
            raise RuntimeError(f"Session {index} out of range (1..{len(sessions)})")
        meta, rows = sessions[index - 1]
        return meta, rows, index
    if selector == "last":
        meta, rows = sessions[-1]
        return meta, rows, len(sessions)
    if selector == "last-connected":
        for offset in range(len(sessions) - 1, -1, -1):
            meta, rows = sessions[offset]
            if rows and rows[-1].get("rx", 0) > 0:
                return meta, rows, offset + 1
        meta, rows = sessions[-1]
        return meta, rows, len(sessions)
    if selector in {"longest", "longest-connected"}:
        candidates = [
            (offset, meta, rows)
            for offset, (meta, rows) in enumerate(sessions)
            if rows and (selector == "longest" or rows[-1].get("rx", 0) > 0)
        ]
        if not candidates:
            raise RuntimeError("No V1-connected sessions found in CSV")
        offset, meta, rows = max(candidates, key=lambda item: (_duration_ms(item[2]), -item[0]))
        return meta, rows, offset + 1
    ...
    def parse_rows(raw_rows: list[str], fields: list[str]) -> list[dict[str, int]]:
        ...
        for raw_line in raw_rows:
            ...
            if len(values) < len(fields):
                raise RuntimeError(
                    f"Truncated CSV row at millis {millis_val}: "
                    f"expected {len(fields)} columns, got {len(values)}"
                )
            row: dict[str, int] = {}
            for index, key in enumerate(fields):
                raw_value = values[index].strip()
                if key == "utc":
                    continue
                row[key] = parse_int(raw_value or "0")
...
def select_segment(
    sessions: list[tuple[Optional[SessionMeta], list[dict[str, int]]]],
    selector: str,
) -> tuple[Optional[SessionMeta], list[dict[str, int]], SessionSummary, list[SessionSummary], str]:
    ...
    if selector == "auto":
        ...
        if drive_candidates:
            ...
            try:
                _meta, _rows, idx = choose_session(sessions, "longest-connected")
            except RuntimeError:
                _meta, _rows, idx = choose_session(sessions, "longest")
    ...
    meta, rows, session_index = choose_session(sessions, selector)
