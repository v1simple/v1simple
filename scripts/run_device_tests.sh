...
skipped_count = sum(int(s.get("skipped_nums", 0)) for s in suites)
...
if failure_count > 0:
    ...
    error_count > 0
    and pass_count > 0
    and pass_count + skipped_count == test_count
):
    status = "PASS"
elif error_count > 0:
    status = "FAIL"
...
if failure_count > 0 or (error_count > 0 and command_status == 0):
    for s in suites:
        if int(s.get("failure_nums", 0)) > 0:
            print(f"  FAILED: {s.get('test_name', 'unknown')}")
        if command_status == 0 and int(s.get("error_nums", 0)) > 0:
            print(f"  ERROR: {s.get('test_name', 'unknown')}")
    print()
    sys.exit(1)
...
if error_count > 0:
    for s in suites:
        if int(s.get("error_nums", 0)) > 0:
            print(f"  INFRA ERROR: {s.get('test_name', 'unknown')} (all assertions passed)")
    print()
PY
}
...
try:
    ...
    cases = [case for s in suites for case in s.get('test_cases', []) if isinstance(case, dict)]
    passes = sum(case.get('status') == 'PASSED' for case in cases)
    skipped = sum(case.get('status') == 'SKIPPED' for case in cases)
    print(
        'yes'
        if total > 0 and passes > 0 and fails == 0
        and len(cases) == total and passes + skipped == total
        else 'no'
    )
except Exception:
    print('no')
