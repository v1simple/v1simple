...
std::vector<uint8_t> makeAlertPayload(uint8_t index, uint8_t count, uint16_t freqMHz, uint8_t frontRaw, uint8_t rearRaw,
    ...
    return std::vector<uint8_t>{indexCount,
...
void assertContainsFrequencies(const PacketParser& parser, uint16_t a, uint16_t b) {
    ...
    for (size_t i = 0; i < count; ++i) {
        if (alerts[i].frequency == a)
            foundA = true;
        if (alerts[i].frequency == b)
            foundB = true;
    }
...
bool parsePacket(PacketParser& parser, const std::vector<uint8_t>& packet, uint32_t nowMs = kDefaultParseNowMs) {
...
void test_alert_stream_ku_raw_flag_is_preserved_without_forcing_mute() {
    ...
    // Ku alerts use the K-band bargraph table so they no longer report 0 bars.
    // The card shows the VR scale unmodified, so this is the VR bar count (4);
    // the old six-bar compression reported 3 for the same raw value.
    TEST_ASSERT_EQUAL_UINT8(4, alerts[0].frontStrength);
...
void test_reset_alert_state_clears_published_and_partial_tables() {
    ...
    const auto stalePartial = makePacket(PACKET_ID_ALERT_DATA, makeAlertPayload(1, 3, 10525, 0x88, 0x00, 0xA8, 0x80));
    ...
    const auto freshRow2 = makePacket(PACKET_ID_ALERT_DATA, makeAlertPayload(2, 3, 33800, 0xA0, 0x00, 0x22, 0x00));
    const auto freshRow3 = makePacket(PACKET_ID_ALERT_DATA, makeAlertPayload(3, 3, 34700, 0xB0, 0x00, 0x22, 0x00));
    TEST_ASSERT_TRUE(parsePacket(parser, freshRow2, 1200));
    TEST_ASSERT_TRUE(parsePacket(parser, freshRow3, 1201));
    TEST_ASSERT_FALSE(parser.hasAlerts());

    const auto freshRow1 = makePacket(PACKET_ID_ALERT_DATA, makeAlertPayload(1, 3, 24150, 0x90, 0x00, 0x24, 0x80));
...
void test_alert_stream_preserves_v1_assignment_and_raw_strengths() {
    PacketParser parser;

    const auto row1 = makePacket(PACKET_ID_ALERT_DATA, makeAlertPayload(1, 2, 34700, 0x91, 0x72, 0x22, 0x80));
    const auto row2 = makePacket(PACKET_ID_ALERT_DATA, makeAlertPayload(2, 2, 24150, 0xA3, 0x84, 0x44, 0x00));

    TEST_ASSERT_TRUE(parsePacket(parser, row1, 100));
    TEST_ASSERT_TRUE(parsePacket(parser, row2, 110));

    const auto& alerts = parser.getAllAlerts();
    TEST_ASSERT_EQUAL_UINT8(1, alerts[0].v1Index);
    TEST_ASSERT_EQUAL_HEX8(0x91, alerts[0].frontRawStrength);
    TEST_ASSERT_EQUAL_HEX8(0x72, alerts[0].rearRawStrength);
    TEST_ASSERT_EQUAL_UINT8(2, alerts[1].v1Index);
    TEST_ASSERT_EQUAL_HEX8(0xA3, alerts[1].frontRawStrength);
    TEST_ASSERT_EQUAL_HEX8(0x84, alerts[1].rearRawStrength);
}

void test_alert_table_observer_receives_only_complete_tables_and_clear() {
    PacketParser parser;
    AlertObserverCapture capture;
    parser.setAlertTableObserver(captureAlertTable, &capture);
    ...
    TEST_ASSERT_EQUAL_UINT32(2, capture.calls);
    TEST_ASSERT_EQUAL_UINT32(0, static_cast<uint32_t>(capture.count));
    TEST_ASSERT_EQUAL_UINT32(200, capture.nowMs);

    TEST_ASSERT_TRUE(parsePacket(parser, clear, 210));
    TEST_ASSERT_EQUAL_UINT32(2, capture.calls);
}
...
void test_strict_alert_stream_duplicate_index_replaces_prior_row() {
    ...
    TEST_ASSERT_TRUE(parsePacket(parser, row1a));
    TEST_ASSERT_TRUE(parsePacket(parser, row1b));
    TEST_ASSERT_EQUAL_UINT32_MESSAGE(0, static_cast<uint32_t>(parser.getAlertCount()),
...
int main() {
    ...
    RUN_TEST(test_alert_stream_preserves_v1_assignment_and_raw_strengths);
