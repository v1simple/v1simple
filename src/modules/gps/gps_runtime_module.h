...
public:
    ...
    void begin(bool enabled, bool enablePinActiveHigh = true,
               uint32_t baud = 9600,
               GpsTimePublisher* timePub = nullptr,
               GpsGeoPublisher* geoPub = nullptr);
    void setEnabled(bool enabled);
    void setBaud(uint32_t baud);  // Updates stored baud; takes effect on next setEnabled(true)
    void setEnablePinActiveHigh(bool activeHigh);  // Updates polarity; takes effect on next setEnabled toggle
...
private:
    ...
    bool enablePinActiveHigh_ = true;  // Polarity flag — true for Adafruit 3133
    bool enabled_ = false;
