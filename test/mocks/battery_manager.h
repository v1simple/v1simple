...
public:
    ...
    bool criticalProtectionRequired() const { return onBattery_ && isCritical(); }
    void update() { ++updateCalls; }
    bool refreshVoltage() {
        ++refreshVoltageCalls;
        if (!refreshVoltageResult) {
            return false;
        }
        if (criticalAfterRefreshEnabled_) {
            setCritical(criticalAfterRefreshValue_);
        }
        hasVoltageReading_ = true;
        lastVoltageReadingMs_ = static_cast<uint32_t>(millis());
        return true;
    }
    bool hasVoltageReadingAfter(uint32_t sinceMs) const {
        return hasVoltageReading_ && static_cast<int32_t>(lastVoltageReadingMs_ - sinceMs) > 0;
    }
    ...
    void setRefreshVoltageResult(bool result) { refreshVoltageResult = result; }
    void setVoltageReadingAt(uint32_t readingMs) {
        hasVoltageReading_ = true;
        lastVoltageReadingMs_ = readingMs;
    }
    void setCriticalAfterRefresh(bool critical) {
        criticalAfterRefreshEnabled_ = true;
        criticalAfterRefreshValue_ = critical;
    }
    void reset() {
        onBattery_ = false;
        hasBattery_ = false;
        ...
        updateCalls = 0;
        refreshVoltageCalls = 0;
        refreshVoltageResult = true;
        criticalAfterRefreshEnabled_ = false;
        criticalAfterRefreshValue_ = false;
        hasVoltageReading_ = false;
        lastVoltageReadingMs_ = 0;
    ...
    int updateCalls = 0;
    int refreshVoltageCalls = 0;
    bool refreshVoltageResult = true;
...
private:
    ...
    bool criticalAfterRefreshEnabled_ = false;
    bool criticalAfterRefreshValue_ = false;
    bool hasVoltageReading_ = false;
    uint32_t lastVoltageReadingMs_ = 0;
};
