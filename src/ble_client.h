...
inline const char* bleStateToString(BLEState state) {
    switch (state) {
        ...
        case BLEState::QUIESCING: return "QUIESCING";
...
public:
    void open(uint32_t generation) {
        token_.store(generation, std::memory_order_release);
    }

    void close() {
        token_.exchange(0, std::memory_order_acq_rel);
    }

    bool accepts(uint32_t generation) const {
        return generation != 0 &&
               token_.load(std::memory_order_acquire) == generation;
    }
    ...
    bool claim(uint32_t generation) {
        ...
        return token_.compare_exchange_strong(
            expected, generation, std::memory_order_acq_rel, std::memory_order_acquire);
    }
...
inline bool bleQuiesceDeadlineExpired(uint32_t nowMs,
                                      uint32_t startedMs,
                                      uint32_t timeoutMs) {
...
public:
    ...
    uint32_t lastV1ConnectionEventMs() const {
        return lastV1ConnectionEventMs_.load(std::memory_order_relaxed);
    }
    bool consumeVerifyPushMatchEdge() {
        return verifyPushMatchEdgePending_.exchange(false, std::memory_order_acq_rel);
    }
