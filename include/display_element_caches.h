...
// and hold stale colors on the panel (Valentine's Law: the display must not
// lie by going stale).
...
//   - slots[2]: active slot data (alert + lastSeen timer) previously
//     scattered as file-scoped statics in display_cards.cpp. Lives here so
//     invalidateAll() actually zeros it on screen clear. The slots carry
