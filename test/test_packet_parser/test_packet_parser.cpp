...
void test_parse_display_packet_laser_keeps_led_bitmap_signal_bars() {
    ...
    const auto packet = makePacket(
        ...
        makeDisplayPayload(63,
                           0x03,  // V1's own LED bitmap: 2 bars
    ...
    TEST_ASSERT_EQUAL_UINT8_MESSAGE(2, state.signalBars,
        "laser display packets must not force synthetic full bars");
}
...
// for stray patterns (e.g. 0x80 → 1 bar instead of "all six"). Pin the spec
// behavior to the V1 Gen2's six-bar display so a future refactor cannot regress.
void test_decode_signal_bars_non_standard_bitmap_returns_full_six() {
    PacketParser parser;
    // image1 = front arrow only, aux0 systemStatus=1 so bands/bars are reported.
    // Standard V1 Gen2 ramp values (0x01,0x03,0x07,...,0x3F) and 0 are the
    // only bitmaps that map to 1..6 / 0 bars. Anything wider or non-contiguous
    // means overflow / show all six bars.
    struct Case { uint8_t bitmap; uint8_t expected; };
    const Case cases[] = {
        {0x80, 6},  // single high bit — popcount would return 1
        {0xAA, 6},  // alternating — popcount would return 4
        {0x55, 6},  // alternating — popcount would return 4
        {0x02, 6},  // single mid bit — popcount would return 1
        {0xF0, 6},  // upper nibble — popcount would return 4
        {0x00, 0},  // explicit zero stays zero
        {0x01, 1}, {0x03, 2}, {0x07, 3}, {0x0F, 4},
        {0x1F, 5}, {0x3F, 6}, {0x7F, 6}, {0xFF, 6},
    };
    for (const auto& c : cases) {
        const auto packet = makePacket(
            PACKET_ID_DISPLAY_DATA,
...
int main(int argc, char** argv) {
    ...
    RUN_TEST(test_decode_signal_bars_non_standard_bitmap_returns_full_six);
    return UNITY_END();
}
