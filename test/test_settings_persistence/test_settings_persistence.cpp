...
void test_fresh_nvs_load_matches_authoritative_constructor_defaults() {
    ...
    TEST_ASSERT_EQUAL_UINT(0u, mock_preferences::missingStringReadCount(kNvsWifiClientSsid));
    TEST_ASSERT_EQUAL_UINT(0u, mock_preferences::missingStringReadCount(kNvsLastV1Address));
...
void test_load_migrates_legacy_one_sided_slot_volume_to_no_change() {
    ...
    TEST_ASSERT_GREATER_THAN(0, active.putInt(kNvsValid, SETTINGS_VERSION));
    TEST_ASSERT_GREATER_THAN(0, active.putInt(kNvsSettingsVer, SETTINGS_VERSION));
    TEST_ASSERT_GREATER_THAN(0, active.putUChar(kNvsSlot0Volume, 7));
    TEST_ASSERT_GREATER_THAN(0, active.putUChar(kNvsSlot0MuteVolume, 0xFF));
    TEST_ASSERT_GREATER_THAN(0, active.putUChar(kNvsSlot1Volume, 6));
    TEST_ASSERT_GREATER_THAN(0, active.putUChar(kNvsSlot1MuteVolume, 2));
    active.end();
    ...
    TEST_ASSERT_EQUAL_UINT8(0xFF, manager.get().slot0MuteVolume);
    TEST_ASSERT_EQUAL_UINT8(6, manager.get().slot1Volume);
    TEST_ASSERT_EQUAL_UINT8(2, manager.get().slot1MuteVolume);
}
...
void test_save_load_and_backup_round_trip_current_shape_fields() {
    ...
    const uint16_t signalBarColors[6] = {0x1001, 0x2002, 0x3003, 0x4004, 0x5005, 0x6006};
    for (int i = 0; i < 6; ++i) {
        settings.colorBars[i] = signalBarColors[i];
    }
    ...
    settings.slot0MuteVolume = 1;
    settings.slot1Volume = 6;
    ...
    for (int i = 0; i < 6; ++i) {
        TEST_ASSERT_EQUAL_HEX16(signalBarColors[i], loaded.colorBars[i]);
    }
    ...
    TEST_ASSERT_EQUAL_INT(SD_BACKUP_VERSION, backupDoc["_version"].as<int>());
    uint16_t compatibilitySegments[8];
    DisplayVisualContract::expandSixBarColorsToEight(signalBarColors, compatibilitySegments);
    for (int i = 0; i < 6; ++i) {
        char key[16];
        std::snprintf(key, sizeof(key), "colorBar%d", i + 1);
        TEST_ASSERT_EQUAL_UINT16(signalBarColors[i], backupDoc[key].as<uint16_t>());
    }
    for (int i = 0; i < 8; ++i) {
        char key[16];
        std::snprintf(key, sizeof(key), "colorBarS%d", i + 1);
        TEST_ASSERT_EQUAL_UINT16(compatibilitySegments[i], backupDoc[key].as<uint16_t>());
    }
...
void test_http_backup_omits_all_wifi_credentials() {
    ...
    manager.mutableSettings().apPassword = "private-ap-secret";
    ...
    TEST_ASSERT_TRUE(exportDoc["apPassword"].isNull());
    TEST_ASSERT_TRUE(exportStaSlots[0]["passwordObf"].isNull());
...
void test_restore_preserves_sta_password_when_sanitized_backup_ssid_matches() {
    ...
    // Restoring that sanitized backup in place must keep the stored password
    // because the slot still names the same network.
    const SettingsBackupApplyResult result = manager.applyBackupDocument(doc, true);
    TEST_ASSERT_TRUE(result.success);
    TEST_ASSERT_EQUAL_STRING("GarageNet", manager.get().wifiStaSlots[0].ssid.c_str());
    TEST_ASSERT_EQUAL_STRING("garage-secret-2026", manager.getWifiStaSlotPassword(0).c_str());
    TEST_ASSERT_TRUE(
        mock_preferences::namespaceHasKey(WIFI_CLIENT_NS, kNvsWifiStaSlotPassword[0]));
}
...
void test_restore_clears_sta_password_when_sanitized_backup_ssid_differs() {
    ...
    const SettingsBackupApplyResult result = manager.applyBackupDocument(doc, true);
    TEST_ASSERT_TRUE(result.success);
    TEST_ASSERT_EQUAL_STRING("OtherNet", manager.get().wifiStaSlots[0].ssid.c_str());
    TEST_ASSERT_FALSE(
        mock_preferences::namespaceHasKey(WIFI_CLIENT_NS, kNvsWifiStaSlotPassword[0]));
}
...
void test_restore_explicit_password_obf_still_overwrites_stored() {
    ...
    slot0["label"] = "Garage";
    slot0["priority"] = 0;
    slot0["passwordObf"] = encodeObfuscatedForStorage("new-secret-2026");

    const SettingsBackupApplyResult result = manager.applyBackupDocument(doc, true);
    TEST_ASSERT_TRUE(result.success);
    TEST_ASSERT_EQUAL_STRING("new-secret-2026", manager.getWifiStaSlotPassword(0).c_str());
}

// NVS partition exhaustion mid-write (32 KB shared pool filling partway
// through a namespace write, not a blanket I/O failure). The firmware's guard
// is the `nvsValid` sentinel written LAST and verified back — with the store
// full before the sentinel lands, the verify must fail and the restore must
// not claim success (a truncated namespace would silently boot as defaults).
...
void test_deferred_restore_reports_failure_when_persist_fails() {
    ...
    // A failed atomic persist must not be reported as a successful restore:
    // the applied settings live only in RAM and vanish on reboot.
    TEST_ASSERT_FALSE(result.success);
}
...
void test_voice_fields_round_trip_through_sd_backup_and_restore() {
    ...
    src.muteVoiceIfVolZero = true;
    src.voiceVolume = 42;
    src.announceSecondaryAlerts = true;
    src.secondaryLaser = false;
    src.secondaryKa = false;
    src.secondaryK = true;
    src.secondaryX = true;
    src.speedMuteVoice = false;
    ...
    TEST_ASSERT_FALSE(backupDoc["voiceDirectionEnabled"].as<bool>());
    TEST_ASSERT_FALSE(backupDoc["announceBogeyCount"].as<bool>());
    TEST_ASSERT_TRUE(backupDoc["muteVoiceIfVolZero"].as<bool>());
    TEST_ASSERT_EQUAL_UINT8(42, backupDoc["voiceVolume"].as<uint8_t>());
    TEST_ASSERT_TRUE(backupDoc["announceSecondaryAlerts"].as<bool>());
    TEST_ASSERT_FALSE(backupDoc["secondaryLaser"].as<bool>());
    TEST_ASSERT_FALSE(backupDoc["secondaryKa"].as<bool>());
    TEST_ASSERT_TRUE(backupDoc["secondaryK"].as<bool>());
    ...
    TEST_ASSERT_EQUAL_INT(VOICE_MODE_FREQ_ONLY, restored.voiceAlertMode);
    TEST_ASSERT_FALSE(restored.voiceDirectionEnabled);
    TEST_ASSERT_FALSE(restored.announceBogeyCount);
    TEST_ASSERT_TRUE(restored.muteVoiceIfVolZero);
    TEST_ASSERT_EQUAL_UINT8(42, restored.voiceVolume);
    TEST_ASSERT_TRUE(restored.announceSecondaryAlerts);
    TEST_ASSERT_FALSE(restored.secondaryLaser);
    TEST_ASSERT_FALSE(restored.secondaryKa);
    TEST_ASSERT_TRUE(restored.secondaryK);
    TEST_ASSERT_TRUE(restored.secondaryX);
    TEST_ASSERT_FALSE(restored.speedMuteVoice);
}
...
// --- six-segment restoration migrations ----------------------------------
...
void test_v18_backup_eight_segment_theme_collapses_to_six() {
    ...
    TEST_ASSERT_TRUE(manager.applyBackupDocument(doc, false).success);
    const uint16_t expected[6] = {0x1001, 0x1002, 0x1004, 0x1005, 0x1007, 0x1008};
    for (int i = 0; i < 6; ++i) {
        TEST_ASSERT_EQUAL_UINT16(expected[i], manager.get().colorBars[i]);
    }
...
void test_v19_backup_direct_colors_win_over_compatibility_shadow() {
    ...
    for (int i = 0; i < 6; ++i) {
        char key[16];
        std::snprintf(key, sizeof(key), "colorBar%d", i + 1);
        doc[key] = static_cast<uint16_t>(0x1100 + i);
    }
    doc["colorBarS1"] = 0xAAAA;
    doc["colorBarS8"] = 0xBBBB;

    TEST_ASSERT_TRUE(manager.applyBackupDocument(doc, false).success);
    for (int i = 0; i < 6; ++i) {
        TEST_ASSERT_EQUAL_UINT16(static_cast<uint16_t>(0x1100 + i), manager.get().colorBars[i]);
    }
}
...
int main() {
    ...
    RUN_TEST(test_restore_preserves_sta_password_when_sanitized_backup_ssid_matches);
    ...
    RUN_TEST(test_v11_eight_segment_theme_collapses_to_six);
    RUN_TEST(test_v12_direct_colors_win_over_compatibility_shadow);
