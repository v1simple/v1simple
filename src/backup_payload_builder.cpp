...
constexpr const char* WIFI_STA_SLOT_PASSWORD_OBF_KEY = "passwordObf";

// computeCrc32 is the canonical IEEE 802.3 CRC32 from settings_backup.cpp.

String readWifiStaSlotPasswordObfForBackup(size_t index) {
    if (index >= kWifiStaSlotCount) {
        return "";
    }

    Preferences prefs;
    if (!prefs.begin(kSettingsWifiClientNamespace, true)) {
        return "";
    }
    bool hasPassword = prefs.isKey(kNvsWifiStaSlotPassword[index]);
    String encoded = hasPassword ? prefs.getString(kNvsWifiStaSlotPassword[index], "") : "";
    if (!hasPassword && index == 0 && prefs.isKey(kNvsWifiPassword)) {
        hasPassword = true;
        encoded = prefs.getString(kNvsWifiPassword, "");
    }
    prefs.end();
    if (encoded.length() == 0) {
        return "";
    ...
    // Copy only decodable non-empty credentials when the caller explicitly
    // opts in. Empty passwords represent open networks and do not need a
    // recovery secret.
    const String decoded = decodeObfuscatedFromStorage(encoded);
    return decoded.length() > 0 ? encoded : "";
}
...
BuildResult buildBackupDocument(JsonDocument& doc,
    ...
    // apPassword is obfuscated the same way it is in NVS so a raw string never
    // sits on SD in cleartext.  applyBackupDocument only applies this when the
    // key is present (clean-flash restore path); in-place restores with an
    // existing NVS will preserve whatever password is already there if the
    // backup is older and lacks this key.
    ...
    for (size_t i = 0; i < kWifiStaSlotCount; ++i) {
        ...
        if (options.includeWifiPasswords && slot.isConfigured()) {
            const String encodedWifiPassword = readWifiStaSlotPasswordObfForBackup(i);
            if (encodedWifiPassword.length() > 0) {
                slotObj[WIFI_STA_SLOT_PASSWORD_OBF_KEY] = encodedWifiPassword;
            }
        }
        wroteStaSlot = true;
    }
    if (!wroteStaSlot && settings.wifiClientSSID.length() > 0) {
        ...
        if (options.includeWifiPasswords) {
            const String encodedWifiPassword = readWifiStaSlotPasswordObfForBackup(0);
            if (encodedWifiPassword.length() > 0) {
                slotObj[WIFI_STA_SLOT_PASSWORD_OBF_KEY] = encodedWifiPassword;
            }
        }
    ...
    doc["gpsEnablePinActiveHigh"] = settings.gpsEnablePinActiveHigh;
    ...
    // Stamp _crc32 on SD backups to catch media-level corruption.
    // HTTP downloads are protected by TLS and do not need an embedded checksum.
