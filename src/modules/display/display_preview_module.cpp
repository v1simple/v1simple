...
const DisplayPreviewModule::PreviewStep DisplayPreviewModule::STEPS[] = {
    ...
    {BAND_X, DIR_FRONT, 10525, 2, 0, FLAG_NONE,
     NO_SEC, NO_THIRD,
     '0', 'A', 0,       // bogey=0 | mode=All Bogeys | profile=Default
    ...
    {BAND_X, DIR_FRONT, 10525, 3, 0, FLAG_NONE,
     BAND_X, DIR_SIDE, 10515, 2, 0,
     BAND_X, DIR_REAR, 10535, 0, 1,
     '5', NO_CHG, NO_CHG,
     NO_CHG, NO_CHG, NO_CHG, NO_CHG,
     NO_CHG, NO_CHG, nullptr},
...
void fillResolvedAlert(DisplayPreviewModule::ResolvedAlert& out,
                       Band band,
                       Direction dir,
                       uint32_t freqMHz,
                       uint8_t frontBars,
                       uint8_t rearBars,
                       const char* alpGunAbbrev = nullptr) {
    ...
    const char* text = DisplayVisualContract::frequencyTextForAlert(
        band, freqMHz, alpGunAbbrev, buffer, sizeof(buffer));
    copyText(out.frequencyText, sizeof(out.frequencyText), text);
}

void buildResolvedStepFromCarry(int index,
                                const DisplayPreviewModule::PreviewStep& step,
    ...
    if (step.secBand != BAND_NONE) {
        fillResolvedAlert(out.secondary,
                          step.secBand,
                          step.secDir,
                          step.secFreqMHz,
                          step.secFrontBars,
    ...
    if (step.flags & DisplayPreviewModule::FLAG_FLASH_ARROW) {
        if (step.dir & DIR_FRONT) out.flashMask |= 0x20;
        if (step.dir & DIR_SIDE) out.flashMask |= 0x40;
        if (step.dir & DIR_REAR) out.flashMask |= 0x80;
    }
...
void DisplayPreviewModule::applyCarryState(PreviewCarryState& state, const PreviewStep& step) {
    if (step.modeChar != NO_CHG) state.modeChar = step.modeChar;
    if (step.profileSlot != NO_CHG) state.profileSlot = step.profileSlot;
    if (step.alpState != NO_CHG) {
        state.alpState = step.alpState;
        state.alpHbByte1 = step.alpHbByte1;
    }
    if (step.obdState != NO_CHG) state.obdState = step.obdState;
    if (step.bleState != NO_CHG) state.bleState = step.bleState;
    if (step.mainVolume != NO_CHG) state.mainVolume = step.mainVolume;
    if (step.muteVolume != NO_CHG) state.muteVolume = step.muteVolume;
}
...
void DisplayPreviewModule::renderResolvedStep(const ResolvedStep& resolved, bool firstFrame) {
    ...
    display_->setAlpPreviewState(alpOn,
                                 static_cast<uint8_t>(resolved.status.alpState),
                                 resolved.status.alpHbByte1);
