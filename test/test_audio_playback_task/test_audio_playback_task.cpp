...
void test_i2s_init_failure_path_uses_caps_delete() {
    ...
    const std::string finishBlock = extractBlock(source, "static void finish_pcm_audio_task()");

    TEST_ASSERT_NOT_EQUAL(std::string::npos, failureBlock.find("finish_pcm_audio_task();"));
    TEST_ASSERT_EQUAL(std::string::npos, failureBlock.find("vTaskDelete(nullptr);"));
    TEST_ASSERT_NOT_EQUAL(std::string::npos, finishBlock.find("audioRecordStackHighWater"));
    TEST_ASSERT_NOT_EQUAL(std::string::npos, finishBlock.find("audioResetTaskState(audio_playing, audioTaskHandle);"));
    TEST_ASSERT_NOT_EQUAL(std::string::npos, finishBlock.find("vTaskDeleteWithCaps(nullptr);"));
}
