...
void test_display_update_gates_base_frame_to_screen_transitions() {
    ...
    // Secondary alert cards were reintroduced (2026-07): the live path must
    // draw the card row and force-invalidate it after a full screen clear.
    TEST_ASSERT_NOT_EQUAL(std::string::npos, liveUpdate.find("drawSecondaryAlertCards"));
    TEST_ASSERT_NOT_EQUAL(std::string::npos, liveUpdate.find("dirty_.cards"));
    TEST_ASSERT_NOT_EQUAL(
        std::string::npos,
...
void test_stale_ble_policy_is_wired_into_display_sources() {
    ...
    TEST_ASSERT_NOT_EQUAL(
        std::string::npos,
        rssiBlock.find("FILL_RECT(rssiRect.x, rssiRect.y, rssiRect.w, rssiRect.h, PALETTE_BG);"));

    const std::string bleBlock = extractBlock(statusSource, "void V1Display::drawBLEProxyIndicator()");
    TEST_ASSERT_NOT_EQUAL(
        std::string::npos,
        bleBlock.find("previewIndicatorOverridesActive_ || hasFreshBleContext(millis())"));
