...
/**
 * Read the six configured bar colors (bar1..bar6) in meter order.
 *
 * Each segment is individually addressable and stored, so this is a plain read
 * rather than a sampling of stops. The firmware paints the same array with no
 * draw-time interpolation (src/display_bands.cpp, src/display_cards.cpp).
 *
 * @param {Record<string, number>} colors  Object with bar1..bar6 RGB565 values
 * @returns {number[]}  6 RGB565 values, index 0 = weakest (bottom bar)
 */
export function sampleBarRampRgb565(colors) {
    return [1, 2, 3, 4, 5, 6].map((n) => colors[`bar${n}`] ?? 0);
}

/**
 * Derive the six stored bar colors (bar1..bar6) from three gradient stops.
 * This is a UI authoring convenience; firmware stores and paints six colors.
 *
 * @param {number} bottom RGB565 (bar 1, weakest)
 * @param {number} middle RGB565 (ramp midpoint)
 * @param {number} top    RGB565 (bar 6, strongest)
 * @returns {Record<string, number>} bar1..bar6
 */
export function deriveBarsFromStops(bottom, middle, top) {
    return {
        bar1: bottom,
        bar2: lerpRgb565(bottom, middle, 2, 5),
        bar3: lerpRgb565(bottom, middle, 4, 5),
        bar4: lerpRgb565(middle, top, 1, 5),
        bar5: lerpRgb565(middle, top, 3, 5),
        bar6: top
    };
}

/**
 * Recover 3 gradient stops from the six stored bar colors, when possible.
 ...
 *
 * This only seeds the simple three-stop editor. Segments stay individually
 * addressable; recovery never rewrites them.
 *
