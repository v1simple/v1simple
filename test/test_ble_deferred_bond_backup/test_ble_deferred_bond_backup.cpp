...
int g_enqueueBackupCalls = 0;
int g_enqueueBackupResult = 1;
...
void V1BLEClient::ClientCallbacks::onConnectFail(NimBLEClient*, int) {}
...
int V1BLEClient::enqueueCurrentBondBackupSnapshot() {
    ++g_enqueueBackupCalls;
    return g_enqueueBackupResult;
}
...
void setUp() {
    mockMillis = 0;
    mockMicros = 0;
    g_enqueueBackupCalls = 0;
    g_enqueueBackupResult = 1;
...
void test_followup_backup_step_marks_pending_without_inline_write() {
    ...
    TEST_ASSERT_EQUAL(0, g_enqueueBackupCalls);
...
void test_service_deferred_bond_backup_retries_after_enqueue_failure() {
    V1BLEClient client;
    ...
    TEST_ASSERT_EQUAL(1, g_enqueueBackupCalls);
    TEST_ASSERT_EQUAL_UINT32(0, StorageManager::mockSdLockState.tryAcquireCalls);
    TEST_ASSERT_EQUAL_UINT32(0, StorageManager::mockSdLockState.blockingAcquireCalls);
...
int main() {
    ...
    RUN_TEST(test_service_deferred_bond_backup_retries_after_enqueue_failure);
