...
**Params:** None.
**Response:** HTML `text/html` (200) or `{ "ok": false, "error": "Web UI not found. Please reflash with ./build.sh --all" }` (500).
**Source:** route registration in `src/wifi_routes.cpp`.

### GET `/_app/*` (catch-all)
**Description:** Serves immutable Svelte app files from `/_app/` with appropriate MIME types (js, css, json). Rejects unsafe paths; falls through to notFound handler if file not found.
**Params:** None.
**Response:** File content with appropriate MIME type (200) or `{ "ok": false, "error": "Not found" }` (404).
**Source:** route registration in `src/wifi_routes.cpp`.
...
**Params:** Color keys as int (via `server.arg()`), boolean toggles as `"true"`/`"1"`, `brightness` (int 0-255), `skipPreview` (bool).
...
**Description:** Generates a JSON backup snapshot — all profiles, settings, catalog. Served as a downloadable attachment. Cached per revision. Default exports include `wifiStaSlots` metadata (`index`, `ssid`, `label`, `priority`, `lastConnectedAtSec`) but omit saved STA passwords.
**Params:** Optional `includePasswords=1|true|on|yes` includes saved STA passwords as obfuscated `passwordObf` fields inside `wifiStaSlots`; use only for device migration and keep the file private.
**Response:** JSON backup file (200); `Content-Disposition: attachment; filename="v1simple_backup.json"` by default or `v1simple_backup_with_wifi_passwords.json` when password export is requested.
...
**Description:** Restores settings from uploaded JSON backup. Max body 128 KB. Validates backup format and applies atomically; triggers post-restore sync.
...
**Response:** `{ "success": true }` (200) or `{ "error": "Saved OBD device not found" }` (404).
**Source:** route registration in `src/wifi_routes.cpp`, delegate `obd_api_service.cpp`.

### POST `/api/obd/scan`
**Description:** Requests a manual OBD device discovery scan.
**Response:** `{ "ok": true, "requested": true, "scanInProgress": bool, "message": "..." }` (200), `{ "error": "maintenance_mode", "message": "OBD runtime endpoints are not available in maintenance mode" }` (409), `{ "ok": false, "message": "OBD is disabled" }` (409), or `{ "ok": false, "message": "OBD scan already requested or in progress" }` (409).
**Source:** route registration in `src/wifi_routes.cpp`, delegate `obd_api_service.cpp`.

### POST `/api/obd/forget`
**Description:** Forgets the saved OBD device.
**Response:** `{ "ok": true }` (200) or `{ "error": "maintenance_mode", "message": "OBD runtime endpoints are not available in maintenance mode" }` (409).
**Source:** route registration in `src/wifi_routes.cpp`, delegate `obd_api_service.cpp`.

### POST `/api/obd/config`
**Description:** Updates OBD configuration; accepts any subset of fields with value clamping.
**Params:** `enabled` (bool), `minRssi` (int -90 to -40), `obdScanWindowMs` (int ≥0), `obdRetryIntervalMs` (int ≥0), `proxyOpenWindowMs` (int ≥0), `wifiOpenTimeoutMs` (int ≥0), `v1SettleQuietMs` (int ≥0), `v1SettleFallbackMs` (int ≥0), `cycleTeardownAckTimeoutMs` (int ≥0).
...
**Response:** `{ "ok": true }` (200), `{ "error": "Invalid JSON", "message": "Invalid JSON" }` (400), or `{ "error": "gps runtime not wired" }` (503).
**Source:** route registration in `src/wifi_routes.cpp`, delegate `gps_api_service.h`.
