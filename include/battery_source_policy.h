...
//
// A single read cannot distinguish external power from a held button. Source
// changes therefore require two agreeing rounds separated in time. Unknown,
// suppressed, and unconfirmed observations resolve to battery because a false
// USB classification would disable low-battery protection. ADC health affects
// voltage reporting only and is not an input to source or button decisions.
//
// Time comparisons use the rollover-safe form:
...
// ─── Power-button gating ──────────────────────────────────────────────────
//
// ADC health and cached-voltage validity remain explicit inputs so tests can
// verify that neither affects button availability.

struct ButtonGateInputs {
    bool managerInitialized = false;
    Source classification = Source::Unknown;
    bool adcHealthy = false;          // Observability only.
    bool batteryVoltageValid = false; // Observability only.
};
