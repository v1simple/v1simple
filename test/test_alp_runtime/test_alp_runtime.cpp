...
void test_alert_active_rearm_resets_timeout_window() {
    ...
    // alertActiveWatchdogMs_ should re-arm to this newer timestamp, so a
...
// Heartbeat-only ALERT_ACTIVE watchdog regressions

void test_heartbeat_only_alert_times_out_from_alert_entry() {
    beginEnabled();
    ...
    processAt(2000 + AlpRuntimeModule::ALERT_ACTIVE_TIMEOUT_MS - 100);
    TEST_ASSERT_EQUAL(AlpState::ALERT_ACTIVE, alpRuntimeModule.getState());
...
void test_repeated_targeted_heartbeat_does_not_rearm_alert_timeout() {
    beginEnabled();
    ...
    // A steady Targeted heartbeat is valid housekeeping, not the 98 trigger
    // rearm required to extend the watchdog window.
    inject(hbAlert, sizeof(hbAlert));
    processAt(2000 + AlpRuntimeModule::ALERT_ACTIVE_TIMEOUT_MS - 100);
    TEST_ASSERT_EQUAL(AlpState::ALERT_ACTIVE, alpRuntimeModule.getState());
...
void test_heartbeat_only_alert_ignores_stale_prior_trigger_timestamp() {
    beginEnabled();

    // Leave a 98-trigger timestamp behind from an earlier completed session.
    inject(BURST_PL3, sizeof(BURST_PL3));
    processAt(1000);
    inject(REG_WRITE_FD, sizeof(REG_WRITE_FD));
    processAt(2000);
    ...
    // Open a much later alert using only heartbeats. Its watchdog must start
    // at this ALERT_ACTIVE entry, not at the old 98 trigger from T=1000.
    const uint8_t hbIdle[] = { 0xB0, 0x03, 0x00, 0x33 };
    const uint8_t hbAlert[] = { 0xB0, 0x01, 0x00, 0x31 };
    inject(hbIdle, sizeof(hbIdle));
    inject(hbAlert, sizeof(hbAlert));
    ...
    processAt(30000 + AlpRuntimeModule::ALERT_ACTIVE_TIMEOUT_MS + 100);
    TEST_ASSERT_EQUAL(AlpState::TEARDOWN, alpRuntimeModule.getState());
}
...
void test_heartbeat_byte1_b8_does_not_trigger_alert() {
    ...
    // B8 frames carry no alert information; only B0 does.
...
void test_heartbeat_alert_reopens_when_listening_inherits_stale_01() {
    ...
    // A stale prevByte1 value must not leave the state machine in LISTENING.
...
// Regression from the alp_18.csv in-engagement teardown re-arm capture.
//
...
// back to ALERT_ACTIVE. Clearing lastGun_ on that transition left the
// remaining ~20s of the same engagement showing generic "LASER".
//
// The gun cache is cleared only when a new engagement begins from LISTENING or
// IDLE. PL2 must remain identified during this in-engagement re-arm.
void test_gun_persists_through_teardown_rearm_cycle() {
    ...
    // A fresh LISTENING → ALERT_ACTIVE transition clears the cache before the
    // CB 10 00 gun frame identifies PL2.
    inject(BURST_PL2_DETECT, sizeof(BURST_PL2_DETECT));
    processAt(37000);
    TEST_ASSERT_EQUAL(AlpState::ALERT_ACTIVE, alpRuntimeModule.getState());
    ...
    // 98 02 00 re-fires during TEARDOWN and re-arms the same engagement.
    const uint8_t observe_trigger[] = { 0x98, 0x02, 0x00, 0x1A };
    inject(observe_trigger, sizeof(observe_trigger));
    processAt(46500);
    TEST_ASSERT_EQUAL(AlpState::ALERT_ACTIVE, alpRuntimeModule.getState());

    // The cached gun identity must survive the re-arm.
    TEST_ASSERT_EQUAL(AlpGunType::LASER_ATLANTA_PL2,
                      alpRuntimeModule.lastIdentifiedGun());
...
void test_session_direction_rear_latches_on_dli_heartbeat() {
    // The direction classifier samples the first non-00 B0 heartbeat *after*
...
void test_latched_rear_session_ignores_later_pdc_heartbeat_for_display() {
    ...
    // Gun-ID burst opens the session AND anchors direction sampling. It must
    // precede the REAR heartbeat.
    inject(BURST_PL3_DETECT, sizeof(BURST_PL3_DETECT));
    processAt(2000);
...
void test_session_direction_relatches_front_over_rear() {
    ...
    // corresponds to the observed FRONT signal (Targeted = RED LED). A REAR
    // latch established by DLI/LID activity must
    // be overridden when the ALP eventually reports Targeted — this is
    // the on-device multi-phase session edge case reproduced in
    // alp_2-f6e0ca7e.csv (PL3 front shot misclassified REAR).
    beginEnabled();
...
// The protocol cache may retain lastGun between engagements, but the display
// projection must return UNKNOWN while the session is inactive.
...
void test_warm_up_flagged_when_preamble_in_window() {
    ...
    // Warm-Up sessions remain hidden from the display projection.
    TEST_ASSERT_FALSE(alpRuntimeModule.hasLaserEvent());
    TEST_ASSERT_FALSE(alpRuntimeModule.currentEvent().active);
    TEST_ASSERT_EQUAL(AlpGunType::UNKNOWN, alpRuntimeModule.currentEvent().gun);
}
...
void test_display_window_stays_open_across_byte1_blip_to_idle() {
    ...
    // The display window remains open while TEARDOWN represents an
    // in-engagement pulse gap.
    TEST_ASSERT_TRUE(alpRuntimeModule.currentSession().active);
    TEST_ASSERT_TRUE(alpRuntimeModule.currentEvent().active);
}
...
void test_direction_relatches_front_on_byte1_targeted() {
    ...
    // latched REAR. Later byte1=01 (Targeted) corresponds to the observed
    ...
    // Gun-ID burst opens the session AND anchors direction sampling. Then LID
...
void test_alp5_post_gun_id_reopen_not_flagged_warmup_in_envelope() {
    ...
    // envelope-only classification would flag every one as Warm-Up and leave
    // the display dark during real laser
    ...
    // the session. alertActiveWatchdogMs_ was set at +12113 ms; stepping to
    ...
    // The next byte1=01 heartbeat opens a new session on the 00→01 edge. A
    // post-identification session must bypass the boot envelope.
    const uint8_t hbTargeted[] = { 0xB0, 0x01, 0x00, 0x31 };
    inject(hbTargeted, sizeof(hbTargeted));
    processAt(33500);
...
int main(int argc, char** argv) {
    ...
    RUN_TEST(test_heartbeat_only_alert_times_out_from_alert_entry);
