...
void raw(Sink& sink, const char* text) {
    sink.write(text, std::strlen(text));
}

template <typename Sink>
void rawLen(Sink& sink, const char* text, size_t len) {
    sink.write(text, len);
}

template <typename Sink>
void quoted(Sink& sink, const char* text) {
    raw(sink, "\"");
    ...
    for (; *s; ++s) {
        switch (*s) {
            ...
            case '\\': raw(sink, "\\\\"); break;
            case '\n': raw(sink, "\\n"); break;
            case '\r': raw(sink, "\\r"); break;
            case '\t': raw(sink, "\\t"); break;
            default: rawLen(sink, s, 1); break;
        }
    }
    raw(sink, "\"");
}
...
template <typename Sink>
void key(Sink& sink, const char* name) {
    quoted(sink, name);
    raw(sink, ":");
}

template <typename Sink>
void number(Sink& sink, long value) {
    char buf[24];
...
template <typename Sink>
void unumber(Sink& sink, unsigned long value) {
    char buf[24];
...
template <typename Sink>
void boolean(Sink& sink, bool value) {
    raw(sink, value ? "true" : "false");
}

template <typename Sink>
void color(Sink& sink, uint16_t value) {
    char buf[10];
...
template <typename Sink>
void quotedChar(Sink& sink, char value) {
    char buf[2] = {value, '\0'};
    quoted(sink, value ? buf : "");
}

template <typename Sink>
void rect(Sink& sink, const DisplayLayout::DisplayRect& r) {
    raw(sink, "{");
    key(sink, "x"); number(sink, r.x); raw(sink, ",");
    key(sink, "y"); number(sink, r.y); raw(sink, ",");
    key(sink, "w"); number(sink, r.w); raw(sink, ",");
    key(sink, "h"); number(sink, r.h);
    raw(sink, "}");
}
...
void settingsFingerprintHex(const V1Settings& settings, char* out, size_t outSize) {
    std::snprintf(out, outSize, "0x%08lX",
                  static_cast<unsigned long>(settingsFingerprint(settings)));
}

template <typename Sink>
void bindingFields(Sink& sink, const Runtime& runtime, const V1Settings& settings) {
    key(sink, "schemaVersion"); number(sink, kManifestSchemaVersion); raw(sink, ",");
    key(sink, "firmwareVersion"); quoted(sink, runtime.firmwareVersion ? runtime.firmwareVersion : "unknown"); raw(sink, ",");
    key(sink, "firmwareSha"); quoted(sink, runtime.firmwareSha ? runtime.firmwareSha : "unknown"); raw(sink, ",");
...
void sendMeasuredJson(WebServer& server, PayloadFn payload) {
...
void stepId(const DisplayPreviewModule::ResolvedStep& step, char* out, size_t outSize) {
    ...
    switch (step.primary.band) {
        case BAND_LASER: band = "laser"; break;
        case BAND_KA: band = "ka"; break;
        case BAND_K: band = "k"; break;
        case BAND_X: band = "x"; break;
        case BAND_KU: band = "ku"; break;
        case BAND_NONE: band = "none"; break;
    }
    std::snprintf(out, outSize, "%03d_%s_%s_%lu",
                  step.index,
                  band,
                  directionName(step.primary.dir),
                  static_cast<unsigned long>(step.primary.freqMHz));
}

template <typename Sink>
void writeFlags(Sink& sink, uint8_t flags) {
    raw(sink, "{");
    key(sink, "muted"); boolean(sink, (flags & DisplayPreviewModule::FLAG_MUTED) != 0); raw(sink, ",");
    key(sink, "photo"); boolean(sink, (flags & DisplayPreviewModule::FLAG_PHOTO) != 0); raw(sink, ",");
    key(sink, "flashArrow"); boolean(sink, (flags & DisplayPreviewModule::FLAG_FLASH_ARROW) != 0); raw(sink, ",");
    key(sink, "flashBand"); boolean(sink, (flags & DisplayPreviewModule::FLAG_FLASH_BAND) != 0);
    raw(sink, "}");
}

template <typename Sink>
void writeRawStep(Sink& sink, const DisplayPreviewModule::PreviewStep& rawStep) {
    raw(sink, "{");
    key(sink, "band"); quoted(sink, bandName(rawStep.band)); raw(sink, ",");
    key(sink, "direction"); quoted(sink, directionName(rawStep.dir)); raw(sink, ",");
    key(sink, "frequencyMHz"); unumber(sink, rawStep.freqMHz); raw(sink, ",");
    key(sink, "frontBars"); unumber(sink, rawStep.frontBars); raw(sink, ",");
    key(sink, "rearBars"); unumber(sink, rawStep.rearBars); raw(sink, ",");
    key(sink, "flags"); writeFlags(sink, rawStep.flags); raw(sink, ",");
    key(sink, "secondaryBand"); quoted(sink, bandName(rawStep.secBand)); raw(sink, ",");
    key(sink, "secondaryDirection"); quoted(sink, directionName(rawStep.secDir)); raw(sink, ",");
    key(sink, "secondaryFrequencyMHz"); unumber(sink, rawStep.secFreqMHz); raw(sink, ",");
    key(sink, "thirdBand"); quoted(sink, bandName(rawStep.thirdBand)); raw(sink, ",");
    key(sink, "thirdDirection"); quoted(sink, directionName(rawStep.thirdDir)); raw(sink, ",");
    key(sink, "thirdFrequencyMHz"); unumber(sink, rawStep.thirdFreqMHz); raw(sink, ",");
    key(sink, "bogeyChar"); number(sink, rawStep.bogeyChar); raw(sink, ",");
    key(sink, "modeChar"); number(sink, rawStep.modeChar); raw(sink, ",");
    key(sink, "profileSlot"); number(sink, rawStep.profileSlot); raw(sink, ",");
    key(sink, "alpState"); number(sink, rawStep.alpState); raw(sink, ",");
    key(sink, "alpHbByte1"); number(sink, rawStep.alpHbByte1); raw(sink, ",");
    key(sink, "obdState"); number(sink, rawStep.obdState); raw(sink, ",");
    key(sink, "bleState"); number(sink, rawStep.bleState); raw(sink, ",");
    key(sink, "mainVolume"); number(sink, rawStep.mainVolume); raw(sink, ",");
    key(sink, "muteVolume"); number(sink, rawStep.muteVolume); raw(sink, ",");
    key(sink, "alpGunAbbrev"); quoted(sink, rawStep.alpGunAbbrev ? rawStep.alpGunAbbrev : "");
    raw(sink, "}");
}

template <typename Sink>
void writeResolvedAlert(Sink& sink, const DisplayPreviewModule::ResolvedAlert& alert) {
    raw(sink, "{");
    key(sink, "present"); boolean(sink, alert.present); raw(sink, ",");
    key(sink, "band"); quoted(sink, bandName(alert.band)); raw(sink, ",");
    key(sink, "direction"); quoted(sink, directionName(alert.dir)); raw(sink, ",");
    key(sink, "frequencyMHz"); unumber(sink, alert.freqMHz); raw(sink, ",");
    key(sink, "frequencyText"); quoted(sink, alert.frequencyText); raw(sink, ",");
    key(sink, "frontBars"); unumber(sink, alert.frontBars); raw(sink, ",");
    key(sink, "rearBars"); unumber(sink, alert.rearBars); raw(sink, ",");
    key(sink, "cardBarCount"); unumber(sink, alert.cardBarCount);
    raw(sink, "}");
}

template <typename Sink>
void writeResolvedStep(Sink& sink,
                       const DisplayPreviewModule::ResolvedStep& step,
                       const V1Settings& settings) {
    auto bandPaletteRole = [](Band band) -> const char* {
        switch (band) {
            case BAND_LASER: return "bands.laser";
            case BAND_KA: return "bands.ka";
            case BAND_K:
            case BAND_KU: return "bands.k";
            case BAND_X: return "bands.x";
            case BAND_NONE: return "gray";
        }
        return "gray";
    };
    ...
    const bool topCounterIsDigit =
        step.status.bogeyChar >= '0' && step.status.bogeyChar <= '9';
    const char* topCounterRole =
        (!topCounterIsDigit && step.muted) ? "muted" : "bogey";
    ...
    raw(sink, "{");
    key(sink, "index"); number(sink, step.index); raw(sink, ",");
    key(sink, "id"); quoted(sink, id); raw(sink, ",");
    key(sink, "raw"); writeRawStep(sink, step.raw); raw(sink, ",");
    key(sink, "resolved"); raw(sink, "{");
    key(sink, "primary"); writeResolvedAlert(sink, step.primary); raw(sink, ",");
    key(sink, "secondary"); writeResolvedAlert(sink, step.secondary); raw(sink, ",");
    key(sink, "third"); writeResolvedAlert(sink, step.third); raw(sink, ",");
    key(sink, "activeBandMask"); unumber(sink, step.activeBandMask); raw(sink, ",");
    key(sink, "activeDirectionMask"); unumber(sink, step.activeDirectionMask); raw(sink, ",");
    key(sink, "flashMask"); unumber(sink, step.flashMask); raw(sink, ",");
    key(sink, "bandFlashMask"); unumber(sink, step.bandFlashMask); raw(sink, ",");
    key(sink, "mainMeterCount"); unumber(sink, step.mainMeterCount); raw(sink, ",");
    key(sink, "alertCount"); unumber(sink, step.alertCount); raw(sink, ",");
    key(sink, "muted"); boolean(sink, step.muted); raw(sink, ",");
    key(sink, "photo"); boolean(sink, step.photo); raw(sink, ",");
    key(sink, "frequencyRole"); quoted(sink, frequencyRole); raw(sink, ",");
    key(sink, "status"); raw(sink, "{");
    key(sink, "bogeyChar"); quotedChar(sink, step.status.bogeyChar); raw(sink, ",");
    key(sink, "modeChar"); quotedChar(sink, step.status.hasMode ? step.status.modeChar : 0); raw(sink, ",");
    key(sink, "profileSlot"); number(sink, step.status.profileSlot); raw(sink, ",");
    key(sink, "alpState"); number(sink, step.status.alpState); raw(sink, ",");
    key(sink, "alpHbByte1"); unumber(sink, step.status.alpHbByte1); raw(sink, ",");
    key(sink, "obdState"); number(sink, step.status.obdState); raw(sink, ",");
    key(sink, "bleState"); number(sink, step.status.bleState); raw(sink, ",");
    key(sink, "mainVolume"); unumber(sink, step.status.mainVolume); raw(sink, ",");
    key(sink, "muteVolume"); unumber(sink, step.status.muteVolume); raw(sink, ",");
    key(sink, "muted"); boolean(sink, step.muted); raw(sink, ",");
    key(sink, "topCounterRole"); quoted(sink, topCounterRole); raw(sink, ",");
    key(sink, "muteBadgeRole"); quoted(sink, step.muted ? "muted" : "background"); raw(sink, ",");
    key(sink, "alpBadgeRole"); quoted(sink, alpBadgeRole); raw(sink, ",");
    key(sink, "obdBadgeRole"); quoted(sink, obdBadgeRole); raw(sink, ",");
    key(sink, "bleBadgeRole"); quoted(sink, bleBadgeRole);
    raw(sink, "}}}");
}

template <typename Sink>
void writeStepsPayload(Sink& sink, const Runtime& runtime, const V1Settings& settings) {
    raw(sink, "{");
    bindingFields(sink, runtime, settings); raw(sink, ",");
    key(sink, "manifest"); quoted(sink, "display-visual-steps"); raw(sink, ",");
    key(sink, "stepCount"); number(sink, DisplayPreviewModule::stepCount()); raw(sink, ",");
    key(sink, "steps"); raw(sink, "[");
    ...
    raw(sink, "],");
    key(sink, "complete"); boolean(sink, true);
    raw(sink, "}");
}

template <typename Sink>
void writePalettePayload(Sink& sink, const V1Settings& s) {
    const ColorPalette& base = ColorThemes::STANDARD();
    uint16_t configured[6] = {
        s.colorBar1, s.colorBar2, s.colorBar3, s.colorBar4, s.colorBar5, s.colorBar6
    };
    uint16_t ramp[8]{};
    DisplayVisualContract::buildMainMeterRamp(configured, ramp);

    key(sink, "palette"); raw(sink, "{");
    key(sink, "background"); color(sink, base.bg); raw(sink, ",");
    key(sink, "text"); color(sink, base.text); raw(sink, ",");
    key(sink, "gray"); color(sink, base.colorGray); raw(sink, ",");
    key(sink, "frequency"); color(sink, s.colorFrequency); raw(sink, ",");
    key(sink, "bogey"); color(sink, s.colorBogey); raw(sink, ",");
    key(sink, "muted"); color(sink, s.colorMuted); raw(sink, ",");
    key(sink, "persisted"); color(sink, s.colorPersisted); raw(sink, ",");
    key(sink, "bands"); raw(sink, "{");
    key(sink, "laser"); color(sink, s.colorBandL); raw(sink, ",");
    key(sink, "ka"); color(sink, s.colorBandKa); raw(sink, ",");
    key(sink, "k"); color(sink, s.colorBandK); raw(sink, ",");
    key(sink, "x"); color(sink, s.colorBandX); raw(sink, ",");
    key(sink, "photo"); color(sink, s.colorBandPhoto); raw(sink, "},");
    key(sink, "arrows"); raw(sink, "{");
    key(sink, "front"); color(sink, s.colorArrowFront); raw(sink, ",");
    key(sink, "side"); color(sink, s.colorArrowSide); raw(sink, ",");
    key(sink, "rear"); color(sink, s.colorArrowRear); raw(sink, "},");
    key(sink, "mainMeterRamp"); raw(sink, "[");
    for (int i = 0; i < 8; ++i) {
        if (i > 0) raw(sink, ",");
        color(sink, ramp[i]);
    }
    raw(sink, "],");
    key(sink, "cardMeterRamp"); raw(sink, "[");
    for (int i = 0; i < 6; ++i) {
        if (i > 0) raw(sink, ",");
        color(sink, configured[i]);
    }
    raw(sink, "],");
    key(sink, "cardMeterDimRamp"); raw(sink, "[");
    for (int i = 0; i < 6; ++i) {
        if (i > 0) raw(sink, ",");
    ...
    raw(sink, "],");
    key(sink, "status"); raw(sink, "{");
    key(sink, "profile0"); color(sink, s.slot0Color); raw(sink, ",");
    key(sink, "profile1"); color(sink, s.slot1Color); raw(sink, ",");
    key(sink, "profile2"); color(sink, s.slot2Color); raw(sink, ",");
    key(sink, "wifiIcon"); color(sink, s.colorWiFiIcon); raw(sink, ",");
    key(sink, "wifiConnected"); color(sink, s.colorWiFiConnected); raw(sink, ",");
    key(sink, "bleConnected"); color(sink, dimColor(s.colorBleConnected, 85)); raw(sink, ",");
    key(sink, "bleConnectedStale"); color(sink, dimColor(s.colorBleConnected, 40)); raw(sink, ",");
    key(sink, "bleDisconnected"); color(sink, dimColor(s.colorBleDisconnected, 85)); raw(sink, ",");
    key(sink, "bleAdvertising"); color(sink, dimColor(s.colorBleDisconnected, 85)); raw(sink, ",");
    key(sink, "volumeMain"); color(sink, s.colorVolumeMain); raw(sink, ",");
    key(sink, "volumeMute"); color(sink, s.colorVolumeMute); raw(sink, ",");
    key(sink, "rssiV1"); color(sink, s.colorRssiV1); raw(sink, ",");
    key(sink, "rssiProxy"); color(sink, s.colorRssiProxy); raw(sink, ",");
    key(sink, "obd"); color(sink, s.colorObd); raw(sink, ",");
    key(sink, "obdAttention"); color(sink, 0xF800); raw(sink, ",");
    key(sink, "alpConnected"); color(sink, s.colorAlpConnected); raw(sink, ",");
    key(sink, "alpDli"); color(sink, s.colorAlpDli); raw(sink, ",");
    key(sink, "alpLidActive"); color(sink, s.colorAlpLidActive); raw(sink, ",");
    key(sink, "alpAlert"); color(sink, s.colorAlpAlert);
    raw(sink, "}}");
}

template <typename Sink>
void writeZone(Sink& sink, const char* id, const DisplayLayout::DisplayRect& r, bool comma) {
    if (comma) raw(sink, ",");
    raw(sink, "{");
    key(sink, "id"); quoted(sink, id); raw(sink, ",");
    key(sink, "rect"); rect(sink, r); raw(sink, ",");
    key(sink, "role"); quoted(sink, "semantic_zone");
    raw(sink, "}");
}

const char* bandRole(int index) {
    switch (index) {
        case 0: return "bands.laser";
        case 1: return "bands.ka";
        case 2: return "bands.k";
        case 3: return "bands.x";
    }
    return "bands.k";
}
...
template <typename Sink>
void writeStringField(Sink& sink, const char* name, const char* value, bool comma = true) {
    key(sink, name); quoted(sink, value);
    if (comma) raw(sink, ",");
}

template <typename Sink>
void writeRectField(Sink& sink, const char* name, const DisplayLayout::DisplayRect& r, bool comma = true) {
    key(sink, name); rect(sink, r);
    if (comma) raw(sink, ",");
}
...
template <typename Sink>
void writeStatusTextElement(Sink& sink,
                            const char* id,
                            const char* source,
                            const DisplayLayout::DisplayRect& r,
                            const char* textRole,
                            const char* roleSource,
                            const char* minCoverage,
                            const char* maxCoverage,
                            bool comma) {
    if (comma) raw(sink, ",");
    raw(sink, "{");
    writeStringField(sink, "id", id);
    ...
    key(sink, "minCoverage"); raw(sink, minCoverage); raw(sink, ",");
    key(sink, "maxCoverage"); raw(sink, maxCoverage);
    raw(sink, "}");
}

template <typename Sink>
void writeLayoutPayload(Sink& sink, const Runtime& runtime, const V1Settings& settings) {
    raw(sink, "{");
    bindingFields(sink, runtime, settings); raw(sink, ",");
    key(sink, "manifest"); quoted(sink, "display-visual-layout"); raw(sink, ",");
    key(sink, "screen"); raw(sink, "{");
    key(sink, "logical"); raw(sink, "{");
    key(sink, "width"); number(sink, V1Display::logicalFramebufferWidth()); raw(sink, ",");
    key(sink, "height"); number(sink, V1Display::logicalFramebufferHeight()); raw(sink, "},");
    key(sink, "raw"); raw(sink, "{");
    key(sink, "width"); number(sink, V1Display::rawFramebufferWidth()); raw(sink, ",");
    key(sink, "height"); number(sink, V1Display::rawFramebufferHeight()); raw(sink, ",");
    key(sink, "format"); quoted(sink, V1Display::rawFramebufferFormat()); raw(sink, ",");
    key(sink, "transform"); quoted(sink, V1Display::rawFramebufferTransform());
    raw(sink, "}},");
    key(sink, "coarseZones"); raw(sink, "[");
    ...
    raw(sink, "],");
    key(sink, "elements"); raw(sink, "{");
    key(sink, "bandCells"); raw(sink, "[");
    for (int i = 0; i < 4; ++i) {
        if (i > 0) raw(sink, ",");
        raw(sink, "{");
        key(sink, "index"); number(sink, i); raw(sink, ",");
        key(sink, "label"); quoted(sink, DisplayVisualContract::bandCellLabel(0, i)); raw(sink, ",");
        writeStringField(sink, "band", bandKey(i));
        const uint8_t bandMask =
            (i == 2)
                ? static_cast<uint8_t>(BAND_K | BAND_KU)
                : DisplayVisualContract::bandCellMask(0, i);
        key(sink, "bandMask"); number(sink, bandMask); raw(sink, ",");
        ...
        key(sink, "minCoverage"); raw(sink, "0.01"); raw(sink, ",");
        key(sink, "inactiveMinCoverage"); raw(sink, "0.01");
        raw(sink, "}");
    }
    raw(sink, "],");
    key(sink, "directionArrows"); raw(sink, "[");
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        key(sink, "directionMask"); number(sink, arrowMasks[i]); raw(sink, ",");
    ...
    raw(sink, "],");
    key(sink, "mainSignalBars"); raw(sink, "[");
    for (int i = 0; i < 8; ++i) {
        if (i > 0) raw(sink, ",");
        raw(sink, "{");
        key(sink, "index"); number(sink, i); raw(sink, ",");
    ...
    key(sink, "minCoverage"); raw(sink, "0.02"); raw(sink, ",");
    key(sink, "maxCoverage"); raw(sink, "0.70");
    raw(sink, "},");
    key(sink, "cardSlots"); raw(sink, "[");
    for (int i = 0; i < 2; ++i) {
        if (i > 0) raw(sink, ",");
        raw(sink, "{");
        key(sink, "slot"); number(sink, i); raw(sink, ",");
        ...
        key(sink, "minCoverage"); raw(sink, "0.02"); raw(sink, ",");
        key(sink, "maxCoverage"); raw(sink, "0.85");
        raw(sink, "}");
    }
    raw(sink, "],");
    key(sink, "cardMeterBars"); raw(sink, "[");
    for (int slot = 0; slot < 2; ++slot) {
        for (int i = 0; i < 6; ++i) {
            if (slot > 0 || i > 0) raw(sink, ",");
            raw(sink, "{");
            key(sink, "slot"); number(sink, slot); raw(sink, ",");
            key(sink, "index"); number(sink, i); raw(sink, ",");
    ...
    raw(sink, "],");
    key(sink, "statusText"); raw(sink, "[");
    writeStatusTextElement(sink, "topCounter", "bogeyChar", DisplayLayout::kTopCounterRect, "bogey", "topCounterRole", "0.02", "0.80", false);
    if (!settings.hideVolumeIndicator) {
        writeStatusTextElement(sink, "volumeMain", "mainVolume", DisplayLayout::volumeMainRect(), "status.volumeMain", nullptr, "0.02", "0.80", true);
        writeStatusTextElement(sink, "volumeMute", "muteVolume", DisplayLayout::volumeMuteRect(), "status.volumeMute", nullptr, "0.02", "0.80", true);
    }
    raw(sink, "],");
    key(sink, "statusBadges"); raw(sink, "[");
    raw(sink, "{");
    ...
    key(sink, "minCoverage"); raw(sink, "0.90"); raw(sink, ",");
    key(sink, "inactiveMinCoverage"); raw(sink, "0.98");
    raw(sink, "},");
    raw(sink, "{");
    ...
    writeStringField(sink, "inactiveRole", "background");
    writeStringField(sink, "activeWhen", "nonzero");
    key(sink, "minCoverage"); raw(sink, "0.05"); raw(sink, ",");
    key(sink, "inactiveMinCoverage"); raw(sink, "0.98");
    raw(sink, "}],");
    key(sink, "ignored"); raw(sink, "[");
    ...
    raw(sink, "]},");
    writePalettePayload(sink, settings); raw(sink, ",");
    key(sink, "overlaps"); raw(sink, "[");
    raw(sink, "{\"id\":\"profile_battery\",\"elements\":[\"profile\",\"batteryIcon\"],\"reason\":\"profile clear can overlap the battery icon corner\"},");
    raw(sink, "{\"id\":\"band_gps\",\"elements\":[\"bandCells[0]\",\"gpsBadge\"],\"reason\":\"top band cell clear can overlap the GPS badge\"},");
    raw(sink, "{\"id\":\"arrow_battery\",\"elements\":[\"directionArrowClusterRaised\",\"batteryPercent\",\"batteryIcon\"],\"reason\":\"raised arrow cluster can overlap battery display regions and requires a restore\"}");
    raw(sink, "],");
    key(sink, "masks"); raw(sink, "[],");
    key(sink, "complete"); boolean(sink, true);
    raw(sink, "}");
}
...
void handleSteps(WebServer& server, const Runtime& runtime) {
    ...
    sendMeasuredJson(server, [&](auto& sink) {
        writeStepsPayload(sink, runtime, settings);
    });
}
...
void handleLayout(WebServer& server, const Runtime& runtime) {
    ...
    const V1Settings& settings = runtime.getSettings(runtime.getSettingsCtx);
    sendMeasuredJson(server, [&](auto& sink) {
        writeLayoutPayload(sink, runtime, settings);
    });
}
