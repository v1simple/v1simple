...
describe('profiles route page', () => {
        ...
        installFetchMock(
            [
                ...
                },
                {
                    method: 'GET',
                    match: '/api/v1/current',
                    respond: jsonResponse({ connected: false, settings: {} })
                }
            ],
            jsonResponse({})
        );
    ...
    it('creates and saves a V1 profile while disconnected', async () => {
        ...
        const fetchMock = installDefaultFetch([
            { method: 'GET', match: '/api/v1/profiles', respond: jsonResponse({ profiles: [] }) },
            {
                method: 'GET',
                match: '/api/v1/current',
                respond: jsonResponse({ connected: false, available: false })
            },
            {
                method: 'POST',
    ...
    it('shows offline authoring and disables V1 push/pull in maintenance mode', async () => {
        ...
        expect(
            screen.getByText(/Read current V1 settings during normal runtime/)
        ).toBeInTheDocument();
        expect(
            screen.getByText(
