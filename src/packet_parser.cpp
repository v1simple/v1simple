...
bool PacketParser::parseDisplayData(const uint8_t* payload, size_t length) {
    ...
    //
    // Valentine's Law note (sanctioned mirror deviation): this debounce delays
    // the MUTED render by one packet, i.e. it errs toward showing the alert at
    // full urgency.  Suppressing urgency early is the failure mode the Law
    // forbids; showing it one frame longer is not.  Do not "fix" this back to
    // a raw mirror without hardware evidence the transients are gone.
    // See docs/VALENTINE_PHILOSOPHY.md (Part II).
    ...
    // This is the authoritative signal strength from V1's own display. The
    // V1G2 hardware presents six visible source bars; this project paints an
    // eight-slot meter, so decodeLEDBitmap() expands source full-scale
    // (0x3F) to our full eight slots. Wider/non-contiguous bitmaps still mean
    // "full" rather than a popcount-style partial strength.
    if (length > 2) {
...
// Decode V1's LED bitmap to the local display bar count (0-8).
// The source bitmap is contiguous from LSB. V1G2 full scale is 0x3F on the
// six visible source bars; our UI has eight slots, so the standard source ramp
// is expanded to 0,1,3,4,5,7,8. This preserves a quiet low end while ensuring
// source full-scale is rendered as full-scale locally.
