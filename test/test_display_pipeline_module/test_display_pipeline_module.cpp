/**
 * Regression boundary: parsed frames preserve and restore the correct display
 * owner across BLE transitions, persisted V1 alerts, and ALP activity.
 */
#include <unity.h>
...
void test_alert_table_waits_for_coherent_display_companion() {
    AlertData oldPriority = makeKAlert();
    oldPriority.direction = DIR_SIDE;
    parser.setAlerts({oldPriority});
    parser.state.activeBands = BAND_K;
    parser.state.arrows = DIR_SIDE;
    parser.state.bogeyCounterChar = '1';
    parser.state.causal.stateRevision = 2;
    parser.state.causal.alertRevision = 1;
    parser.state.causal.stateSource.eventSeq = 11;
    parser.state.causal.stateSource.dutMillis = 1000;
    parser.state.causal.alertSource.eventSeq = 10;
    parser.state.causal.alertSource.dutMillis = 999;

    module.handleParsed(1001);
    TEST_ASSERT_EQUAL(1, display.renderFrameCalls);
    TEST_ASSERT_EQUAL(BAND_K, display.lastRenderFrame.v1Priority.band);

    // A complete Ka table arrives before its matching display packet.  This is
    // the exact ordering that produced a physical 34.700 + stale K/side frame
    // in the 200 fps bench video.
    AlertData newPriority = makeKaAlert(34700);
    newPriority.isPriority = true;
    newPriority.direction = DIR_FRONT;
    parser.setAlerts({newPriority});
    parser.state.causal.stateRevision = 3;
    parser.state.causal.alertRevision = 2;
    parser.state.causal.stateSource.eventSeq = 12;
    parser.state.causal.stateSource.dutMillis = 1100;
    parser.state.causal.alertSource.eventSeq = 12;
    parser.state.causal.alertSource.dutMillis = 1100;

    module.handleParsed(1101);
    module.refreshBlinkTick(1150);
    TEST_ASSERT_TRUE(module.restoreCurrentOwner(1160));
    TEST_ASSERT_EQUAL(1, display.forceNextRedrawCalls);
    TEST_ASSERT_EQUAL_MESSAGE(1, display.renderFrameCalls,
                              "table-only, blink, and owner-restore paths must retain the last coherent panel frame");

    // The companion display packet makes the snapshot coherent and releases
    // the new frame immediately.
    parser.state.activeBands = BAND_KA;
    parser.state.arrows = DIR_FRONT;
    parser.state.bogeyCounterChar = '1';
    parser.state.causal.stateRevision = 4;
    parser.state.causal.stateSource.eventSeq = 13;
    parser.state.causal.stateSource.dutMillis = 1165;

    module.handleParsed(1166);
    TEST_ASSERT_EQUAL(2, display.renderFrameCalls);
    TEST_ASSERT_EQUAL(BAND_KA, display.lastRenderFrame.v1Priority.band);
    TEST_ASSERT_EQUAL(DIR_FRONT, display.lastRenderFrame.primaryState.arrows);
}

void test_missing_display_companion_fails_open_after_bounded_hold() {
    AlertData priority = makeKaAlert(34700);
    priority.isPriority = true;
    parser.setAlerts({priority});
    parser.state.activeBands = BAND_K;
    parser.state.arrows = DIR_SIDE;
    parser.state.bogeyCounterChar = '1';
    parser.state.causal.stateRevision = 2;
    parser.state.causal.alertRevision = 1;
    parser.state.causal.stateSource.eventSeq = 20;
    parser.state.causal.stateSource.dutMillis = 1000;
    parser.state.causal.alertSource.eventSeq = 20;
    parser.state.causal.alertSource.dutMillis = 1000;

    module.handleParsed(1099);
    TEST_ASSERT_EQUAL(0, display.renderFrameCalls);
    TEST_ASSERT_FALSE(module.companionFrameRefreshDue(1099));
    TEST_ASSERT_FALSE_MESSAGE(module.companionFrameRefreshDue(1100),
                              "an old causal timestamp must not make the fallback immediately due");
    TEST_ASSERT_TRUE(module.companionFrameRefreshDue(1199));

    // Re-publishing the same contradiction must not extend the bound merely
    // because its parser event identity changed.
    parser.state.causal.stateRevision = 3;
    parser.state.causal.alertRevision = 2;
    parser.state.causal.stateSource.eventSeq = 21;
    parser.state.causal.stateSource.dutMillis = 1150;
    parser.state.causal.alertSource.eventSeq = 21;
    parser.state.causal.alertSource.dutMillis = 1150;
    module.handleParsed(1150);
    TEST_ASSERT_TRUE(module.companionFrameRefreshDue(1199));

    // A materially different contradiction gets its own bounded companion
    // window instead of inheriting one millisecond from the prior mismatch.
    parser.state.activeBands = BAND_X;
    parser.state.causal.stateRevision = 4;
    parser.state.causal.alertRevision = 3;
    parser.state.causal.stateSource.eventSeq = 22;
    parser.state.causal.stateSource.dutMillis = 1198;
    parser.state.causal.alertSource.eventSeq = 22;
    parser.state.causal.alertSource.dutMillis = 1198;
    module.handleParsed(1198);
    TEST_ASSERT_FALSE(module.companionFrameRefreshDue(1199));
    TEST_ASSERT_TRUE(module.companionFrameRefreshDue(1298));

    module.refreshBlinkTick(1298);
    TEST_ASSERT_EQUAL_MESSAGE(1, display.renderFrameCalls,
                              "the scheduled fallback must release a live alert without another parsed packet");
}

void test_owner_restore_timeout_runs_full_deferred_frame_path() {
    AlertData priority = makeKaAlert(34700);
    priority.isPriority = true;
    parser.setAlerts({priority});
    parser.state.activeBands = BAND_K;
    ...
    parser.state.causal.stateRevision = 2;
    parser.state.causal.alertRevision = 1;
    parser.state.causal.stateSource.eventSeq = 40;
    parser.state.causal.stateSource.dutMillis = 1000;
    parser.state.causal.alertSource.eventSeq = 40;
    parser.state.causal.alertSource.dutMillis = 1000;

    module.handleParsed(1001);
    TEST_ASSERT_EQUAL(0, display.renderFrameCalls);
    TEST_ASSERT_EQUAL(0, g_voiceProcessCalls);
    TEST_ASSERT_FALSE(alertPersistence.getPersistedAlert().isValid);

    TEST_ASSERT_TRUE(module.restoreCurrentOwner(1101));
    TEST_ASSERT_EQUAL(1, display.forceNextRedrawCalls);
    TEST_ASSERT_EQUAL(1, display.renderFrameCalls);
    TEST_ASSERT_EQUAL(1, g_voiceProcessCalls);
    TEST_ASSERT_EQUAL_UINT16(34700, alertPersistence.getPersistedAlert().frequency);
}

void test_owner_restore_coherent_companion_runs_full_deferred_frame_path() {
    AlertData priority = makeKaAlert(34700);
    priority.isPriority = true;
    priority.direction = DIR_FRONT;
    parser.setAlerts({priority});
    parser.state.activeBands = BAND_K;
    parser.state.arrows = DIR_SIDE;
    parser.state.bogeyCounterChar = '1';
    parser.state.causal.stateRevision = 2;
    parser.state.causal.alertRevision = 1;
    parser.state.causal.stateSource.eventSeq = 50;
    parser.state.causal.stateSource.dutMillis = 1000;
    parser.state.causal.alertSource.eventSeq = 50;
    parser.state.causal.alertSource.dutMillis = 1000;
    ...
    // The matching display packet was parsed while another presentation owned
    // the panel, so owner restoration is the first chance to consume it.
    parser.state.activeBands = BAND_KA;
    parser.state.arrows = DIR_FRONT;
    parser.state.causal.stateRevision = 3;
    parser.state.causal.stateSource.eventSeq = 51;
    parser.state.causal.stateSource.dutMillis = 1050;

    TEST_ASSERT_TRUE(module.restoreCurrentOwner(1051));
    TEST_ASSERT_EQUAL(1, display.forceNextRedrawCalls);
    TEST_ASSERT_EQUAL(1, display.renderFrameCalls);
    TEST_ASSERT_EQUAL(1, g_voiceProcessCalls);
    TEST_ASSERT_EQUAL_UINT16(34700, alertPersistence.getPersistedAlert().frequency);
...
void test_blink_refresh_coherent_companion_runs_full_deferred_frame_path() {
    AlertData priority = makeKaAlert(34700);
    priority.isPriority = true;
    priority.direction = DIR_FRONT;
    parser.setAlerts({priority});
    ...
    parser.state.causal.stateRevision = 2;
    parser.state.causal.alertRevision = 1;
    parser.state.causal.stateSource.eventSeq = 60;
    parser.state.causal.stateSource.dutMillis = 1000;
    parser.state.causal.alertSource.eventSeq = 60;
    parser.state.causal.alertSource.dutMillis = 1000;
    ...
    parser.state.arrows = DIR_FRONT;
    parser.state.causal.stateRevision = 3;
    parser.state.causal.stateSource.eventSeq = 61;
    parser.state.causal.stateSource.dutMillis = 1050;

    module.refreshBlinkTick(1051);
    TEST_ASSERT_EQUAL(1, display.renderFrameCalls);
    TEST_ASSERT_EQUAL(1, g_voiceProcessCalls);
    TEST_ASSERT_EQUAL_UINT16(34700, alertPersistence.getPersistedAlert().frequency);
}

void test_display_first_pair_releases_when_later_alert_table_agrees() {
    AlertData oldPriority = makeKAlert();
    oldPriority.direction = DIR_SIDE;
    parser.setAlerts({oldPriority});
    parser.state.activeBands = BAND_K;
    ...
    parser.state.causal.stateRevision = 2;
    parser.state.causal.alertRevision = 1;
    parser.state.causal.stateSource.eventSeq = 30;
    parser.state.causal.stateSource.dutMillis = 1000;
    parser.state.causal.alertSource.eventSeq = 29;
    parser.state.causal.alertSource.dutMillis = 999;
    module.handleParsed(1001);
    TEST_ASSERT_EQUAL(1, display.renderFrameCalls);
    ...
    // The display packet arrives first and disagrees with the old K table.
    parser.state.activeBands = BAND_KA;
    parser.state.arrows = DIR_FRONT;
    parser.state.causal.stateRevision = 3;
    parser.state.causal.stateSource.eventSeq = 31;
    parser.state.causal.stateSource.dutMillis = 1100;
    module.handleParsed(1101);
    TEST_ASSERT_EQUAL(1, display.renderFrameCalls);

    // Publishing the matching table advances both causal sources to the table
    // event. The now-coherent pair must render immediately, not wait 100 ms.
    parser.setAlerts({newPriority});
    parser.state.causal.stateRevision = 4;
    parser.state.causal.alertRevision = 2;
    parser.state.causal.stateSource.eventSeq = 32;
    parser.state.causal.stateSource.dutMillis = 1120;
    parser.state.causal.alertSource.eventSeq = 32;
    parser.state.causal.alertSource.dutMillis = 1120;
    module.handleParsed(1121);
    TEST_ASSERT_EQUAL(2, display.renderFrameCalls);
    TEST_ASSERT_EQUAL(BAND_KA, display.lastRenderFrame.v1Priority.band);
    TEST_ASSERT_EQUAL(DIR_FRONT, display.lastRenderFrame.primaryState.arrows);
}
...
void test_handle_parsed_synthesizes_laser_alert_when_alp_active_and_no_v1() {
    ...
    // The live ALP projection is delivered atomically.
...
int main() {
    ...
    RUN_TEST(test_alert_table_waits_for_coherent_display_companion);
    RUN_TEST(test_missing_display_companion_fails_open_after_bounded_hold);
    RUN_TEST(test_owner_restore_timeout_runs_full_deferred_frame_path);
    RUN_TEST(test_owner_restore_coherent_companion_runs_full_deferred_frame_path);
    RUN_TEST(test_blink_refresh_coherent_companion_runs_full_deferred_frame_path);
    RUN_TEST(test_display_first_pair_releases_when_later_alert_table_agrees);
