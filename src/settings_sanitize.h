...
inline bool isConfiguredSlotVolumePair(uint8_t volume, uint8_t muteVolume) {
    return volume <= 9 && muteVolume <= 9;
}

inline void sanitizeSlotVolumePair(uint8_t& volume, uint8_t& muteVolume) {
    volume = clampSlotVolumeValue(volume);
    muteVolume = clampSlotVolumeValue(muteVolume);

    // REQWRITEVOLUME always writes both values. A legacy one-sided pair cannot
    // be applied safely without first reading an authoritative companion value,
    // so migrate it to the atomic "do not change" state.
    if (!isConfiguredSlotVolumePair(volume, muteVolume)) {
        volume = 0xFF;
        muteVolume = 0xFF;
    }
}
