...
bool isAllowedLogPath(const char* path) {
    ...
    constexpr const char* ENCOUNTER_PREFIX = "/encounters/";
    ...
    if (hasPrefix(path, ENCOUNTER_PREFIX)) {
        return isSafeLeaf(path + std::strlen(ENCOUNTER_PREFIX));
    }
    return false;
}
...
void handleApiList(WebServer& server, const Runtime& runtime) {
    ...
    if (fileCount < MAX_LISTED_FILES) {
        appendDirectory(files, *runtime.filesystem, "/encounters", "encounters", fileCount, scannedEntryCount,
                        truncated);
    } else {
        truncated = true;
    }
