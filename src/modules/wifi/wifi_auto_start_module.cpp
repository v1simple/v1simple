...
WifiAutoStartDecisionSnapshot
WifiAutoStartModule::buildDecisionSnapshot(unsigned long nowMs, unsigned long v1ConnectedAtMs, bool enableWifi,
                                           bool bleConnected, bool canStartDma, bool wifiAutoStartAllowed,
                                           bool wifiManualStartIntentLatched, bool wifiAutoStartDone,
                                           bool startTriggered, bool startSucceeded) const {
    ...
    snapshot.msSinceV1Connect = (v1ConnectedAtMs > 0 && static_cast<int32_t>(nowMs - v1ConnectedAtMs) >= 0)
...
void WifiAutoStartModule::logDecisionIfChanged(const WifiAutoStartDecisionSnapshot& snapshot) {
    ...
    if (hasLoggedDecision_ && snapshot.gate == lastLoggedGate_ &&
    ...
    SerialLog.printf("[WiFiAutoStart] gate=%s enableWifi=%s bleConnected=%s "
                     ...
                     wifiAutoStartGateName(snapshot.gate), snapshot.enableWifi ? "on" : "off",
                     snapshot.bleConnected ? "yes" : "no", static_cast<unsigned long>(snapshot.msSinceV1Connect),
                     snapshot.canStartDma ? "yes" : "no", snapshot.wifiAutoStartDone ? "yes" : "no",
                     snapshot.bleSettled ? "yes" : "no", snapshot.bootTimeoutReached ? "yes" : "no",
                     snapshot.shouldAutoStart ? "yes" : "no", snapshot.startTriggered ? "yes" : "no");
...
bool WifiAutoStartModule::process(unsigned long nowMs, unsigned long v1ConnectedAtMs, bool enableWifi,
                                  bool bleConnected, bool canStartDma, bool wifiAutoStartAllowed,
                                  bool& wifiManualStartIntentLatched, bool& wifiAutoStartDone,
                                  bool (*startWifi)(bool autoStarted, void* ctx), void* ctx) {
    lastDecision_ =
        buildDecisionSnapshot(nowMs, v1ConnectedAtMs, enableWifi, bleConnected, canStartDma, wifiAutoStartAllowed,
                              wifiManualStartIntentLatched, wifiAutoStartDone, false, false);
    ...
    if (!startSucceeded) {
        lastDecision_ =
            buildDecisionSnapshot(nowMs, v1ConnectedAtMs, enableWifi, bleConnected, canStartDma, wifiAutoStartAllowed,
                                  wifiManualStartIntentLatched, wifiAutoStartDone, true, false);
        logDecisionIfChanged(lastDecision_);
        return false;
    }
    ...
    lastDecision_ =
        buildDecisionSnapshot(nowMs, v1ConnectedAtMs, enableWifi, bleConnected, canStartDma, wifiAutoStartAllowed,
                              wifiManualStartIntentLatched, wifiAutoStartDone, true, true);
    logDecisionIfChanged(lastDecision_);
    return true;
}
