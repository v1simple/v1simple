<script>
	...
	let gpsEnablePinActiveHigh = $state(true);
	...
	async function fetchGpsConfig() {
		try {
			...
			if (typeof data.gpsEnablePinActiveHigh === 'boolean')
				gpsEnablePinActiveHigh = data.gpsEnablePinActiveHigh;
	...
	async function handlePolarityChange(checked) {
		gpsEnablePinActiveHigh = checked;
		await saveField({ gpsEnablePinActiveHigh });
	}
		...
		{#if loaded}
			...
			{#if gpsEnabled && $isMaintenance}
				...
				<div class="surface-note copy-muted text-sm">
					Live GPS fix data is not running in maintenance mode. GPS settings remain editable and
					apply on next normal boot.
				</div>
						...
						<span
							class={gpsStatus.stableHasFix
								...
								: gpsStatus.hasFix
									...
									: 'text-error'}
						>
							{gpsStatus.stableHasFix ? 'Stable' : gpsStatus.hasFix ? 'Acquiring' : 'None'}
						</span>
