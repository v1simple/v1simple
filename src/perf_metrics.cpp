...
void perfRecordV1LedBitmapAnomaly() {
    // Saturate rather than wrap: an anomaly count that rolls over to zero would
    // read as "clean" on exactly the noisy link that produced it.
    if (perfExtended.v1LedBitmapAnomalies < UINT32_MAX) {
        ++perfExtended.v1LedBitmapAnomalies;
    }
}
