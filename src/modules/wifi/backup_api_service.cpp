...
static void sendBackup(WebServer& server,
    ...
    const bool includeWifiPasswords =
        server.hasArg("includePasswords") && isTruthyArgValue(server.arg("includePasswords"));
    if (includeWifiPasswords) {
        Serial.println("[HTTP] WARN: exporting backup with WiFi STA passwords by explicit request");
        server.sendHeader("Content-Disposition",
                          "attachment; filename=\"v1simple_backup_with_wifi_passwords.json\"");
        WifiJson::Document doc;
        const uint32_t snapshotMs =
            millisFn ? millisFn(millisCtx) : static_cast<uint32_t>(millis());
        if (runtime.buildDocumentWithOptions) {
            runtime.buildDocumentWithOptions(doc, snapshotMs, true, runtime.ctx);
        } else if (runtime.buildDocument) {
            runtime.buildDocument(doc, snapshotMs, runtime.ctx);
        }
