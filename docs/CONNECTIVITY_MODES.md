...
- `proxyOpenWindowMs` is a passive/fallback timing knob. Explicit Proxy / App
  mode is excluded from the proxy-window timeout entirely (see
  `isExplicitProxyAppMode` in `connection_cycle_coordinator_module.cpp`) —
  the open window never auto-closes on timeout in that mode.
