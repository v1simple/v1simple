<script>
	...
	function openWifiEditor(slotIndex = null, seed = {}) {
		wifiEditor = {
			...
			ssid: seed.ssid || '',
			password: '',
			priority: Number.isFinite(Number(seed.priority)) ? Number(seed.priority) : nextWifiPriority(),
			showPassword: false,
	...
	async function saveWifiEditor() {
		...
		try {
			...
			if (wifiEditor.password || wifiEditor.mode !== 'edit' || !wifiEditor.hasExistingPassword) {
	...
	async function moveWifiSlot(slot, direction) {
		...
		[reordered[position], reordered[targetPosition]] = [reordered[targetPosition], reordered[position]];
	...
	async function downloadBackup({ includePasswords = false } = {}) {
		try {
			const res = await fetchWithTimeout(includePasswords
				? '/api/settings/backup?includePasswords=1'
				: '/api/settings/backup');
			if (res.ok) {
				...
				a.href = url;
				a.download = includePasswords
					? 'v1simple_backup_with_wifi_passwords.json'
					: 'v1simple_backup.json';
				document.body.appendChild(a);
				a.click();
				document.body.removeChild(a);
				window.URL.revokeObjectURL(url);
				message = { type: 'success', text: includePasswords
					? 'Backup with WiFi passwords downloaded!'
					: 'Backup downloaded!' };
			} else {
	...
	async function downloadBackupWithPasswords() {
		if (!confirm('Warning: this export includes saved WiFi passwords. Only use it to migrate devices, and keep the file private. Continue?')) {
			return;
		}
		await downloadBackup({ includePasswords: true });
	}
	...
	{:else}
		...
		<div class="surface-card">
			<div class="card-body space-y-4">
				<CardSectionHead title="WiFi Client" subtitle="Save up to four STA networks for maintenance-mode auto-join.">
					<input
						type="checkbox"
						class="toggle toggle-primary"
				...
				{#if wifiStatus.enabled}
					{#if wifiStatus.state === 'connected'}
						<StatusAlert message={{ type: 'success', text: wifiConnectedStatusText() }} />
					...
					{:else}
						<StatusAlert message={{ type: 'info', text: 'Saved networks will be tried in priority order while AP stays online.' }} />
					{/if}
					...
					<div class="space-y-3">
						...
						{#if !wifiNetworksLoading && !hasConfiguredWifiSlots()}
							<div class="surface-note copy-muted">
								No saved WiFi networks yet. Add a car hotspot, phone hotspot, or garage AP so future maintenance sessions can auto-join.
							</div>
						{/if}
								...
								<div class="rounded-box border border-base-300 bg-base-100/70 p-3">
										...
										<div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
											<div class="min-w-0 space-y-1">
												<div class="flex flex-wrap items-center gap-2">
													<span class="font-semibold">{slot.label || slot.ssid}</span>
													{#if isSlotConnected(slot)}
														<span class="badge badge-success badge-sm">Connected</span>
														<span class="badge badge-outline badge-sm">{wifiStatus.rssi} dBm</span>
													{:else if wifiStatus.state === 'connecting' && (wifiStatus.savedSSID === slot.ssid || wifiStatus.connectedSSID === slot.ssid)}
														<span class="badge badge-info badge-sm">Connecting</span>
													{:else}
														<span class="badge badge-ghost badge-sm">Saved</span>
													{/if}
												</div>
												<p class="copy-muted text-sm">SSID: {slot.ssid}</p>
												<p class="copy-muted text-xs">
													Slot {slot.index + 1} • Priority {slot.priority + 1}
													{#if slot.hasPassword}
														• password saved
													{:else}
											...
											<div class="flex flex-wrap gap-2">
												<button class="btn btn-ghost btn-xs" onclick={() => openWifiEditorForSlot(slot)}>
													Edit
												</button>
												<button
												...
												<button class="btn btn-error btn-outline btn-xs" onclick={() => deleteWifiSlot(slot)}>
													Delete
												</button>
											</div>
										</div>
									{:else}
										<div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
											<div>
												<p class="font-semibold">Slot {slot.index + 1}: Empty slot</p>
												<p class="copy-muted text-sm">Save another maintenance network here.</p>
											</div>
											<div class="flex flex-wrap gap-2">
												<button class="btn btn-primary btn-xs" onclick={() => openWifiEditor(slot.index)}>
													Enter manually
												</button>
												<button class="btn btn-outline btn-xs" onclick={() => startWifiScan(slot.index)}>
													Pick from scan
												</button>
											</div>
										</div>
			...
			{#if wifiEditor.open}
				<div class="modal modal-open">
					<div class="modal-box surface-modal max-w-md">
						...
						<div class="py-4 space-y-4">
							...
							<div class="field-control">
								...
								<div class="join w-full">
									<input
										...
										placeholder={wifiEditor.mode === 'edit' && wifiEditor.hasExistingPassword
									...
									<button
										...
										onclick={() => (wifiEditor.showPassword = !wifiEditor.showPassword)}
									>
								...
								{#if wifiEditor.mode === 'edit' && wifiEditor.hasExistingPassword}
									<div class="label">
										<span class="field-hint">Leave blank to keep the saved password.</span>
									</div>
								{/if}
							</div>
							...
							<div class="field-control">
								...
								/>
								<div class="label">
									<span class="field-hint">0 is tried first; higher numbers are lower priority.</span>
								</div>
							</div>
						</div>

						<div class="modal-action">
							<button class="btn btn-ghost" onclick={closeWifiEditor} disabled={wifiNetworkActionInFlight}>
								Cancel
							</button>
							<button
					...
					<div class="modal-backdrop bg-black/50" role="presentation" onclick={closeWifiEditor}></div>
				</div>
			{/if}
		...
		<SettingsBackupCard
				...
				ondownloadBackupWithPasswords={downloadBackupWithPasswords}
				{restoreFile}
				{restoring}
