// AXS15231B touch support derived from the Waveshare reference implementation.
...
TouchHandler::TouchHandler()
    : i2cAddr_(AXS_TOUCH_ADDR), rstPin_(-1), touchActive_(false), lastTouchTime_(0), lastReleaseTime_(0),
      touchDebounceMs_(200), releaseDebounceMs_(100) {}
...
bool TouchHandler::getTouchPoint(int16_t& x, int16_t& y) {
    ...
    uint8_t err = Wire.endTransmission(false); // Repeated start for the response read.
    ...
    // Response: gesture, point count, then 12-bit X and Y coordinates.
...
uint8_t TouchHandler::readRegister(uint8_t reg) {
    Wire.beginTransmission(i2cAddr_);
    Wire.write(reg);
    uint8_t err = Wire.endTransmission(false); // Repeated start for the register read.
