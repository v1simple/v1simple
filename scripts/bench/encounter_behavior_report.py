...
def _payload(result):
    payload = {key: deepcopy(result[key]) for key in (
        ...
        "comparison", "coverage", "scope", "errors", "status", "result") if key in result}
    payload["events"] = []
    for event in result.get("events", []):
        item = {key: deepcopy(event[key]) for key in (
            "event_id", "input_key", "input", "wire_rows", "target", "findings", "coverage",
            "start_ns", "end_ns", "outcome", "status", "comparison") if key in event}
...
<div class="identity" id="identity"></div><div class="stats" id="stats"></div>
...
const inputText=e=>{const rows=e.wire_rows||e.input?.rows||e.input;if(Array.isArray(rows))return rows.length?rows.map(r=>(r.priority?'Primary input: ':'Additional input: ')+valueText(r)).join('\n'):'No alert rows';return typeof rows==='string'?rows:rows?valueText(rows):typeof e.input_key==='string'?e.input_key:'Input details unavailable';};
...
const findingList=e=>e.findings||[];
...
function identity(){const evidence=report.evidence||{},runtime=evidence.runtime_identity||{},source=evidence.tooling_source||{};
 ...
 $('stats').innerHTML=[[events.length,'authored input events'],[target+' / '+events.length,'complete target observed'],[persistent,'events with explained differences'],[unknown.toLocaleString(),'unresolved field readings retained']].map(([n,label])=>'<div class="stat"><b>'+esc(n)+'</b><span>'+esc(label)+'</span></div>').join('');
 ...
 $('method').innerHTML='<p>'+esc(report.scope||report.behavior_contract?.scope||'Visible V1 display behavior in the controlled replay.')+'</p><p>Tooling and firmware identities are separate. A target seen once is not proof that the rest of the event remained correct.</p><details><summary>Recorded identity</summary><pre>'+esc(json(runtime))+'</pre></details><details><summary>Bound settings</summary><pre>'+esc(json(evidence.configuration||{}))+'</pre></details><details><summary>Reader and tooling identity</summary><pre>'+esc(json({reader:report.reader_method,tooling:source}))+'</pre></details><details><summary>Collection and analysis coverage</summary><pre>'+esc(json(report.coverage||{}))+'</pre></details>';
...
function renderIndex(){const query=$('search').value.toLowerCase(),filter=$('filter').value;
 const visible=events.map((e,i)=>({e,i})).filter(({e})=>{const match=!query||JSON.stringify([e.event_id,e.input_key,e.input,e.wire_rows,e.target,e.findings]).toLowerCase().includes(query);return match&&(filter==='all'||filter==='findings'&&findingList(e).length||filter==='unknown'&&unresolved(e)||filter==='missing'&&!ob(e).target_observed||filter==='observed'&&ob(e).target_observed);});
 $('list-count').textContent=visible.length+' of '+events.length+' events';
 $('event-list').innerHTML=visible.map(({e,i})=>{const finds=findingList(e).length,seen=ob(e).target_observed;return '<button class="event-button '+(i===activeIndex?'active':'')+'" data-event="'+i+'"><b>'+esc(e.event_id)+'</b><span class="tag '+(finds?'bad':seen?'':'warn')+'">'+esc(finds?finds+' finding'+(finds===1?'':'s'):seen?'Target observed':'Target not seen')+'</span><span>'+esc(inputText(e).slice(0,130))+'</span></button>';}).join('')||'<div class="empty">No events match this filter.</div>';
}
...
function selectEvent(index){if(!events[index])return;activeIndex=index;const e=events[index],o=ob(e),first=o.first_target_observation;renderIndex();
 ...
 for(const w of [first,...findingList(e).flatMap(f=>[f.first,f.last]),...Object.values(o.fields||{}).flatMap(f=>[f.first_target_observation,f.latest_contrary_observation,f.end_state?.last_observation,...(f.unresolved_intervals||[]).flatMap(u=>[u.first,u.last])])])if(isFrame(w)&&!seenFrames.has(frameNo(w))){activeFrames.push(w);seenFrames.add(frameNo(w));frameMap.set(frameNo(w),w);}
 activeFrames.sort((a,b)=>frameNo(a)-frameNo(b));
 const expected=e.target?.fields||e.target||{},count=findingList(e).length;
 $('event-content').innerHTML='<div class="event-heading"><h2>'+esc(e.event_id)+'</h2><span class="tag '+(count?'bad':o.target_observed?'':'warn')+'">'+esc(count?count+' explained finding'+(count===1?'':'s'):o.target_observed?'Complete target observed':'Complete target not observed')+'</span></div><div class="facts"><div><h3>Controlled input</h3><pre class="reading literal">'+esc(inputText(e))+'</pre><details><summary>Accepted input details</summary><pre>'+esc(plain(e.input||e.wire_rows||e.input_key||{}))+'</pre></details></div><div><h3>Expected display</h3>'+expectedTable(expected)+'</div></div><h3>Observed response</h3><p>'+(first?'The first complete target was read at '+witness(e,first)+'.':'No complete target was read in this event. Individual matching, differing and unreadable fields remain below.')+'</p>'+(o.target_already_correct_in_preceding_observation?'<p class="notice">The target also matched the preceding image. This does not establish a new visual response to this input.</p>':'')+'<p class="small muted">Offsets use complete host input as zero. Partial transitions are retained as measurements, not automatically treated as firmware faults.</p>'+findingList(e).map(f=>findingHTML(e,f)).join('')+'<details><summary>Event coverage and ending boundary</summary><pre>'+esc(json(e.coverage||o.coverage||{}))+'</pre></details>';
 renderFields(e);renderSources(e);const selected=first&&activeFrames.findIndex(s=>frameNo(s)===frameNo(first));showFrame(Number.isInteger(selected)&&selected>=0?selected:0);}
...
function renderComparison(){const comparison=report.comparison;if(!comparison)return;const changes=comparison.events||comparison.event_comparisons||[];
 $('comparison').innerHTML='<section class="panel"><h2>Firmware comparison</h2><p>'+esc(comparison.basis||comparison.reason||'Like-for-like event comparison from the same observation method.')+'</p><details><summary>Comparison evidence</summary><pre>'+esc(json(comparison.summary||comparison))+'</pre></details>'+(Array.isArray(changes)&&changes.length?'<p class="small">'+changes.length+' event comparisons retained in the result.</p>':'')+'</section>';
}
document.addEventListener('click',event=>{const eventButton=event.target.closest('[data-event]');if(eventButton){selectEvent(Number(eventButton.dataset.event));return;}const frameButton=event.target.closest('[data-frame]');if(frameButton)jumpFrame(Number(frameButton.dataset.frame));});
...
identity();renderComparison();renderIndex();if(events.length)selectEvent(0);else{$('event-content').innerHTML='<h2>No event observations</h2><p>The result contains no evaluated input events. See analysis limitations above.</p>';$('viewer').hidden=true;$('field-content').hidden=true;$('source-content').hidden=true;}
</script></body></html>'''
