...
| File | Role |
|---|---|
| `wifi_static_path_guard.{h,cpp}` | Path-traversal guard for `/_app/*` static files. |
...
Path-traversal guard for `/_app/*` requests — rejects `..`, absolute paths, etc.
...
`WifiStaticPathGuard::isSafe` is the one place path-traversal is checked. If you add a new static-file route, route it through that guard. Bypassing it is a security bug.
