...
bool isWifiProcessingEnabledPolicy(const WiFiManager& wifiManager) {
    return wifiManager.isWifiServiceActive() || wifiManager.hasPendingLifecycleWork() || wifiManager.isConnected();
}
...
void WifiPriorityPolicyModule::apply(unsigned long nowMs, V1BLEClient& bleClient, WiFiManager& wifiManager) {
    ...
    const unsigned long uiTimeoutMs =
        wifiPriorityCurrent ? WIFI_PRIORITY_DISABLE_TIMEOUT_MS : WIFI_PRIORITY_ENABLE_TIMEOUT_MS;
