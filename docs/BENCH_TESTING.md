...
**Prerequisite: a working microSD card in the board's TF slot.** The perf
logger records the bench CSV to SD and auto-enables only when the SD_MMC mount
succeeds; without a card the qualification protocol rejects `QSTART` with
`perf_sd_disabled` and the run exits `COLLECTION_FAILED` (3). A failed mount
shows as `sdmmc_init_ocr: send_op_cond ... 0x107` in `bench_serial.log`. Use a
