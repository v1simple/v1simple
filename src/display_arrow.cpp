...
DisplayLayout::DisplayRect V1Display::arrowBoundingRect(bool raisedLayout) {
    const int cxBase = SCREEN_WIDTH - 70 - 6;   // matches cx assignment in drawDirectionArrow
    ...
    // D7 fix: in raised live layout the cluster extends right of the previous
    // SCREEN_WIDTH-42 clip line. Truncating the clear there left a ~37 px
    // stale band on the right edge of every arrow — see arrow_diag04 + memory
    // note display_arrow_partial_flush_clip_note_20260429.md. Honest the
    // bounding rect to the actual glyph extent; the live drawDirectionArrow
    // path restores the battery icon (which lives at SCREEN_WIDTH-50,0,48,30)
    // in the same frame so the right corner doesn't go blank.
    if (!raisedLayout) {
    ...
    r.x = static_cast<int16_t>(clearLeft);
    r.y = static_cast<int16_t>(totalTop);
    r.w = static_cast<int16_t>(clearWidth);
    r.h = static_cast<int16_t>(totalBottom - totalTop);
    return r;
}
