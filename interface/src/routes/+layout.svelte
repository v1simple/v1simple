...
<div class="app-shell">
	...
	<nav class="navbar surface-chrome border-b" aria-label="Main navigation">
		<div class="navbar-start">
			...
			<button type="button" aria-label="Open navigation menu" aria-expanded={menuOpen} class="btn btn-ghost lg:hidden" onclick={toggleMenu}>
			...
			<ul class="menu menu-sm dropdown-content mt-3 z-[1] w-52 surface-menu" role="menu">
					{#each navLinks as link}
						<li><a href={link.href} class="nav-link" class:active={isActivePath(link.href)} onclick={closeMenu}>{link.label}</a></li>
					{/each}
				</ul>
			</div>
