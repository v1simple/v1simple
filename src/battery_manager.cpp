...
bool BatteryManager::begin() {
    ...
    const bool adcInitialized = initADC();
    if (!adcInitialized) {
    ...
    if (adc1_handle && readADCMillivolts(initialVoltage)) {
    ...
    // TCA9554 initialization occurs earlier on the battery path.
...
bool BatteryManager::readADCMillivolts(uint16_t& voltage) {
    if (!adc1_handle)
        return false;
...
bool BatteryManager::hasBattery() const {
    // A nonzero simulated voltage represents a battery in diagnostic mode.
    if (simulatedVoltage_ > 0) {
        return true;
    }

    // Physical battery detection requires an initialized ADC.
    if (!initialized_ || !adc1_handle) {
        return false;
    }

    // A connected USB supply suppresses the battery icon.
    if (!onBattery_) {
        return false;
    }

    // Reject a floating-high source pin when voltage is below the cell floor.
...
bool BatteryManager::refreshVoltage() {
    if (!initialized_ || simulatedVoltage_ > 0) {
        return false;
    }

    uint16_t voltage = 0;
    if (!readADCMillivolts(voltage)) {
        return false;
    }

    cachedVoltage_ = voltage;
    cachedPercent_ = battery_math::voltageToPercent(voltage);
    lastUpdateMs_ = static_cast<uint32_t>(millis());
    hasVoltageReading_ = true;
    return true;
}

bool BatteryManager::hasVoltageReadingAfter(uint32_t sinceMs) const {
    return hasVoltageReading_ && static_cast<int32_t>(lastUpdateMs_ - sinceMs) > 0;
}
...
bool BatteryManager::criticalProtectionRequired() const {
    return battery_math::criticalProtectionRequired(onBattery_, cachedVoltage_);
}
...
bool BatteryManager::powerOff() {
    ...
    if (strategy == poweroff_policy::Strategy::DEEP_SLEEP_EXTERNAL_POWER) {
        ...
        bool batteryLatchReadbackSucceeded = false;
        bool batteryLatchLow = false;
        if (batteryLatchDropped) {
            ...
            if (readbackResult == AudioI2cResult::Ok) {
                batteryLatchReadbackSucceeded = true;
                batteryLatchLow = (readback & (1 << TCA9554_PWR_LATCH_PIN)) == 0;
            }
        }
        const battery_latch_shutdown_policy::Decision batteryLatchDecision = battery_latch_shutdown_policy::evaluate(
            {batteryLatchDropped, batteryLatchReadbackSucceeded, batteryLatchLow});
        const char* batteryLatchOutcome =
            battery_latch_shutdown_policy::externalOutcomeName(batteryLatchDecision.verification);
    ...
    const battery_latch_shutdown_policy::Decision latchDecision =
        battery_latch_shutdown_policy::evaluate({latchDropped, latchReadbackSucceeded, latchLow});
    const char* fallbackReason = battery_latch_shutdown_policy::batteryFallbackReason(latchDecision.verification);
    if (latchDecision.waitForRailCollapse) {
    ...
    // Reset its budget before waiting for a held power button to release.
    (void)esp_task_wdt_reset();

    // Wait for the manual PWR hold to release before fallback sleep. GPIO16 is safe as an
...
bool BatteryManager::processPowerButton() {
    // Button availability depends on the confirmed power source, not ADC
    // health; an ADC failure degrades voltage reporting only.
    ...
    // adc1_handle is null when ADC initialization fails.
    ...
    const bool physicalPinLow = isPowerButtonPressed();
    // Require at least one released sample before arming so a button held at
    // power-on cannot trigger an immediate shutdown.
    if (!physicalPinLow)
    ...
    const bool result = processPowerButtonState(physicalPinLow, nowMs, state);
