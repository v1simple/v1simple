...
#include "display_segments.h"  // SegMetrics, segMetrics() for non-349 profile path
...
void V1Display::drawVolumeIndicator(uint8_t mainVol, uint8_t muteVol) {
    ...
    drawnRegion_.add(static_cast<int16_t>(x), static_cast<int16_t>(y),
                     static_cast<uint16_t>(clearW), static_cast<uint16_t>(clearH),
...
void V1Display::drawRssiIndicator(int rssi) {
    ...
    auto clearLine = [&](int lineY) {
        drawnRegion_.add(static_cast<int16_t>(x), static_cast<int16_t>(lineY),
                         static_cast<uint16_t>(clearW), static_cast<uint16_t>(lineHeight),
                         DisplayDirtyRegionSource::Status);
        FILL_RECT(x, lineY, clearW, lineHeight, PALETTE_BG);
    };
...
void V1Display::drawProfileIndicator(int slot) {
    ...
    const int cx = SCREEN_WIDTH - 70 - 6;  // Same as arrow cx
    const int y = 152;                      // Below arrows
    const int clearW = 130;                 // Wide enough for profile names at size 2
    const int clearH = 20;                  // Tall enough for size 2 font
    ...
    if (s.hideProfileIndicator && !inFlashPeriod) {
        ...
        drawnRegion_.add(static_cast<int16_t>(cx - clearW/2), static_cast<int16_t>(y),
                         static_cast<uint16_t>(clearW), static_cast<uint16_t>(clearH),
                         DisplayDirtyRegionSource::Status);
...
void V1Display::drawBatteryIndicator() {
    ...
    if (s.showBatteryPercent && !s.hideBatteryIcon && batteryManager.hasBattery()) {
        ...
        if (!elementCaches_.battery.iconValid) {
            ...
            drawnRegion_.add(static_cast<int16_t>(battX - 2), static_cast<int16_t>(battY - capH - 4),
                             static_cast<uint16_t>(battW + 4), static_cast<uint16_t>(battH + capH + 6),
                         DisplayDirtyRegionSource::Status);
        ...
        if (!s_batteryShowOnUSB) {
            ...
            if (elementCaches_.battery.lastPctVisible) {
                perfRecordDisplayStatusPaint(PerfDisplayStatusPaint::Battery);
                drawnRegion_.add(static_cast<int16_t>(SCREEN_WIDTH - 50), 0, 48, 30,
                         DisplayDirtyRegionSource::Status);
...
void V1Display::drawBLEProxyIndicator() {
    ...
    if (!bleProxyEnabled_) {
        if (bleProxyDrawn_) {
            ...
            drawnRegion_.add(static_cast<int16_t>(bleX - 2), static_cast<int16_t>(bleY - 2),
                             static_cast<uint16_t>(iconSize + 4), static_cast<uint16_t>(iconSize + 4),
                         DisplayDirtyRegionSource::Status);
...
void V1Display::drawWiFiIndicator() {
    ...
    if (s.hideWifiIcon) {
        ...
        drawnRegion_.add(static_cast<int16_t>(wifiX - 2), static_cast<int16_t>(wifiY - 2),
                         static_cast<uint16_t>(wifiSize + 4), static_cast<uint16_t>(wifiSize + 4),
                         DisplayDirtyRegionSource::Status);
