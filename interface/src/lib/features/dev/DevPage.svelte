...
<div class="page-stack">
	<PageHeader title="Development Settings" subtitle="Local-only maintenance toggles. HTTP debug metrics and perf-file endpoints are intentionally disabled; use SD logs instead." />
	...
	{:else}
			...
			<div class="card-body">
				...
				<div class="field-control">
					<label class="label cursor-pointer">
						<div>
							<span class="field-label font-semibold">Hide default password banner</span>
				...
				<div class="field-control">
					<label class="label cursor-pointer">
						<div>
							<span class="field-label font-semibold">Auto-accept development warning</span>
							<p class="copy-caption-soft mt-1">Hide the advanced warning banner and keep maintenance controls unlocked.</p>
						</div>
						<input
							type="checkbox"
							class="toggle toggle-primary"
		...
		<div class="surface-card" class:opacity-50={!acknowledged}>
			<div class="card-body">
				<CardSectionHead title="Power Diagnostics" subtitle="Maintenance-only shutdown logging for hardware validation." />
		...
		{#if nvsDiag}
			<div class="surface-card">
				<div class="card-body">
					<CardSectionHead title="NVS Persistence" subtitle="Reads directly from flash — confirms settings survive reboot." />
					<div class="grid grid-cols-2 gap-2 text-sm">
						<span class="opacity-60">Status</span>
						<span class={nvsDiag.healthy ? 'text-success font-bold' : 'text-error font-bold'}>
		...
		<div class="surface-card">
			<div class="card-body">
				<CardSectionHead title="Runtime Diagnostics" subtitle="Use SD logs instead of live HTTP debug polling." />
				<p class="copy-caption-soft">
					The HTTP debug metrics, scenario playback, and perf-file endpoints are disabled to reduce maintenance-server load. Perf CSV logging still writes to SD for offline analysis.
				</p>
			</div>
		</div>
