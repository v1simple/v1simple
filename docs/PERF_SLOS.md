...
| `C-SIGNAL-LIVE` | Live simple signal bars receive the parsed V1 LED-bar bitmap count. | Trace event with `sourceSignalBars`, `renderedSignalBars`, `signalBarsStatus`; native render-operation test verifies signal-bar strength reaches `drawVerticalSignalBars()`. Physical bar pixels remain future LCD coverage. | measured-render-frame+native-render-operation |
