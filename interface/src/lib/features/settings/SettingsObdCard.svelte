<script>
	...
	async function fetchObdConfig({ showLoadError = false } = {}) {
		try {
			...
			if (typeof data.obdRetryIntervalMs === 'number') obdRetryIntervalMs = data.obdRetryIntervalMs;
			if (typeof data.proxyOpenWindowMs === 'number') proxyOpenWindowMs = data.proxyOpenWindowMs;
			if (typeof data.wifiOpenTimeoutMs === 'number') wifiOpenTimeoutMs = data.wifiOpenTimeoutMs;
			if (typeof data.v1SettleQuietMs === 'number') v1SettleQuietMs = data.v1SettleQuietMs;
			if (typeof data.v1SettleFallbackMs === 'number') v1SettleFallbackMs = data.v1SettleFallbackMs;
...
<div class="surface-card">
	<div class="card-body space-y-4">
		<CardSectionHead title="OBD-II Speed Source" subtitle="Connect an OBDLink CX for vehicle speed data. Enabling OBD disables proxy mode." />
		...
		{#if enabled}
			...
			<div class="surface-note space-y-3">
				<div class="copy-caption font-semibold uppercase tracking-wide">Connection Cycle</div>
				<div class="grid gap-3 md:grid-cols-2">
					<div class="field-control">
						...
						<input
							...
							max="60000"
							disabled={saving}
							onchange={() => handleTimingChange('obdScanWindowMs', obdScanWindowMs)}
						/>
					</div>
					...
					<div class="field-control">
						...
						<input
							...
							max="600000"
							disabled={saving}
							onchange={() => handleTimingChange('obdRetryIntervalMs', obdRetryIntervalMs)}
						/>
					</div>
					...
					<div class="field-control">
						...
						<input
							...
							max="300000"
							disabled={saving}
							onchange={() => handleTimingChange('proxyOpenWindowMs', proxyOpenWindowMs)}
						/>
					</div>
					...
					<div class="field-control">
						...
						<input
							...
							max="120000"
							disabled={saving}
							onchange={() => handleTimingChange('wifiOpenTimeoutMs', wifiOpenTimeoutMs)}
						/>
					</div>
					...
					<div class="field-control">
						...
						<input
							...
							max="5000"
							disabled={saving}
							onchange={() => handleTimingChange('v1SettleQuietMs', v1SettleQuietMs)}
						/>
					</div>
					...
					<div class="field-control">
						...
						<input
							...
							max="10000"
							disabled={saving}
							onchange={() => handleTimingChange('v1SettleFallbackMs', v1SettleFallbackMs)}
						/>
					</div>
					...
					<div class="field-control">
						...
						<input
							...
							max="1000"
							disabled={saving}
							onchange={() => handleTimingChange('cycleTeardownAckTimeoutMs', cycleTeardownAckTimeoutMs)}
						/>
					</div>
				</div>
			</div>
			...
			{#if $isMaintenance}
				<div class="surface-note copy-muted text-sm">
					OBD live state is not running in maintenance mode. Settings and saved adapters remain editable
					and apply on next normal boot.
				</div>
			{:else if obdStatus}
