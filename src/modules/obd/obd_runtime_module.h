...
class ObdRuntimeModule {
  ...
  private:
    ...
    void resetForBegin();
    // Provisions the BLE client slot and the transport task/queues. Called only
    // from the enabled paths (begin() with enabled, setEnabled(true)) so a
    // disabled OBD costs no NimBLE client slot and no 8KB transport task.
