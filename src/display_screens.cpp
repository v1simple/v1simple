...
void V1Display::showBootSplash() {
    ...
    const unsigned long splashTextStartMs = millis();
    GFX_setTextDatum(MC_DATUM);
    TFT_CALL(setTextSize)(5);
    TFT_CALL(setTextColor)(PALETTE_TEXT, PALETTE_BG);
    GFX_drawString(tft_, "V1", SCREEN_WIDTH / 2, 58);

    TFT_CALL(setTextSize)(3);
    TFT_CALL(setTextColor)(0x07E0, PALETTE_BG);  // Green accent
    GFX_drawString(tft_, "SIMPLE", SCREEN_WIDTH / 2, 102);

    TFT_CALL(setTextSize)(1);
    TFT_CALL(setTextColor)(PALETTE_GRAY, PALETTE_BG);
    GFX_drawString(tft_, "Valentine One Gen2 Display", SCREEN_WIDTH / 2, 132);
    const unsigned long splashTextMs = millis() - splashTextStartMs;
    ...
    Serial.printf("[BootTiming] splash total=%lu text=%lu flush=%lu\n",
