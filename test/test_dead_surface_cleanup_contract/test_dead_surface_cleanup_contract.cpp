...
// Secondary alert cards were removed as a dead surface, then deliberately
// reintroduced (2026-07, ported from the legacy v1g2_simple tree). The
// contract flips from "fully removed" to "fully wired": the surface must not
// exist in a half-state where geometry, cache, renderer, or call sites are
// present without the others.
void test_secondary_alert_cards_surface_is_fully_wired() {
    const std::string displayHeader = readTextFile("src/display.h");
    ...
    TEST_ASSERT_TRUE(pathExists("src/display_cards.cpp"));
    TEST_ASSERT_FALSE_MESSAGE(displayHeader.empty(), "failed to read src/display.h");
    ...
    TEST_ASSERT_NOT_EQUAL(std::string::npos, displayHeader.find("drawSecondaryAlertCards"));
    TEST_ASSERT_NOT_EQUAL(std::string::npos, displayLayout.find("SECONDARY_ROW_HEIGHT"));
    TEST_ASSERT_NOT_EQUAL(std::string::npos, displayLayout.find("SECONDARY_ROW_Y"));
    TEST_ASSERT_NOT_EQUAL(std::string::npos, displayLayout.find("kSecondaryCardsRect"));
    TEST_ASSERT_NOT_EQUAL(std::string::npos, displayCaches.find("CardsRenderCache"));
    TEST_ASSERT_NOT_EQUAL(std::string::npos, displayUpdate.find("PerfDisplayRenderSubphase::Cards"));
    TEST_ASSERT_NOT_EQUAL(std::string::npos, displayUpdate.find("drawSecondaryAlertCards"));
    TEST_ASSERT_NOT_EQUAL(std::string::npos, displayScreens.find("drawSecondaryAlertCards"));
    // Primary content must stay above the cards row (frequency bumped up).
    TEST_ASSERT_NOT_EQUAL(std::string::npos,
                          displayLayout.find("constexpr int CONTENT_BOTTOM_Y = SECONDARY_ROW_Y;"));
}
...
int main() {
    ...
    RUN_TEST(test_secondary_alert_cards_surface_is_fully_wired);
