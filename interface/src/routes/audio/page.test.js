...
describe('audio route page', () => {
	...
	it('loads audio settings from the dedicated audio API', async () => {
		...
		expect(screen.getByText('Settings save to the device immediately and are live during normal drive mode.')).toBeInTheDocument();
	...
	it('shows load error and disables saving when the settings request returns an HTTP error', async () => {
		const fetchMock = installFetchMock(
			[{ method: 'GET', match: '/api/audio/settings', respond: jsonResponse({ error: 'boom' }, 500) }],
			jsonResponse({})
		);
		const { unmount } = render(Page);
