...
class TempReleaseRepo:
    ...
    def write_release_bump(self, bump: str) -> None:
...
class PrepareReleaseTests(unittest.TestCase):
    ...
    def test_auto_bump_consumes_minor_policy_once_then_returns_to_patch(self) -> None:
        self.repo.write_version("1.0.25")
        self.repo.commit_all("baseline")
        self.repo.git("tag", "v1.0.25")
        self.repo.write_release_bump("minor")
        self.repo.commit_all("select next minor release")

        result, values = self.repo.prepare("auto")

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(values["version"], "1.1.0")
        self.assertEqual(values["bump"], "minor")
        self.assertEqual(
            (self.repo.root / ".release-bump").read_text(encoding="utf-8"),
            "patch\n",
        )

        self.repo.commit_all("prepare v1.1.0")
        self.repo.git("tag", "v1.1.0")
        self.repo.add_change("next patch content")

        next_result, next_values = self.repo.prepare("auto")

        self.assertEqual(next_result.returncode, 0, next_result.stderr)
        self.assertEqual(next_values["version"], "1.1.1")
        self.assertEqual(next_values["bump"], "patch")
        self.assertEqual(
            (self.repo.root / ".release-bump").read_text(encoding="utf-8"),
            "patch\n",
        )

    def test_auto_bump_rejects_invalid_reviewed_policy_without_writing(self) -> None:
        self.repo.commit_all("baseline")
        self.repo.git("tag", "v1.0.1")
        self.repo.write_release_bump("preview")
        self.repo.commit_all("invalid release policy")
        self.repo.add_change()
        original_config = (self.repo.root / "include" / "config.h").read_text(
            encoding="utf-8"
        )
